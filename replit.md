...
- **Platform media fallback**: `apps/backend/src/api/platform/media/route.ts` serves Vercel Blob images in production and falls back to local seed images in dev when Blob is unavailable. Category-aware fallback mapping (fashion→content, electronics→content, food→grocery, etc.).
