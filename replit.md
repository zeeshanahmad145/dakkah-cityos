...
- **Detail enrichment**: `apps/backend/src/lib/detail-enricher.ts` — `enrichDetailItem(item, module)` applied to ALL 60+ detail routes' DB-returned paths. Normalizes `%2F`-encoded thumbnails, provides category fallback images, and injects 5 default reviews when DB records lack them. Idempotent — preserves existing thumbnails/reviews if present.
- **Platform media fallback**: `apps/backend/src/api/platform/media/route.ts` serves Vercel Blob images in production and falls back to local seed images in dev when Blob is unavailable. 18 category-aware fallback mappings (fashion, electronics, home, food, grocery, beauty, sports, fitness, automotive, education, real-estate, travel, pet, restaurant, office, luxury, outdoor, default).
- **Collections API wrapper**: `apps/backend/src/api/store/collections/route.ts` enriches Medusa core collections with seed thumbnails. Falls back to hardcoded collection data if DB query fails.
...
- **Seeded customer accounts**: 9 customer accounts with working email/password auth via Medusa's emailpass provider. All have Saudi addresses. Login: `{name}@dakkah.com` / `Dakkah2024!` (mohammed, fatima, ahmed, khalid, sara, omar, noura, abdullah, test_customer). Admin: `admin@dakkah.sa` / `admin123456`.
...
- **Per-customer DB data**: All 9 customers have seeded data across 19+ tables: wallet(9), wallet_transaction(16), wishlist(9), wishlist_item(9), booking(12), ticket(9), audit_log(13), analytics_event(9), dispute(3), permit(5), purchase_order(5), ins_policy(8), trade_in_request(6), enrollment(8), donation(6). B2B: company(5), invoice(6), quote(6). Sub-tables: purchase_order_item(30), invoice_item(36), quote_item(31), company_user(25), membership(18), subscription(18), point_transaction(25).
