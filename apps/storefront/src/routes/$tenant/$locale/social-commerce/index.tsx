...
export const Route = createFileRoute("/$tenant/$locale/social-commerce/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse social commerce on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/social-commerce/posts`,
        {
          headers: {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm text-white/70 mb-4">
            ...
            <span className="text-white">
              {t(locale, "socialCommerce.breadcrumb")}
            </span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "socialCommerce.hero_title")}
          </h1>
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            <span>
              {items.length} {t(locale, "socialCommerce.shops_count")}
            </span>
            <span>|</span>
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "socialCommerce.search_label")}
                </label>
                <input
                  type="text"
                  value={searchQuery}
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "socialCommerce.platform_label")}
                </label>
                <div className="space-y-1">
                  {platformOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "verticals.all_platforms")
                        : `${platformIcons[opt] || ""} ${opt.charAt(0).toUpperCase() + opt.slice(1)}`}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "socialCommerce.category_label")}
                </label>
                <div className="space-y-1">
            ...
            {filteredItems.length === 0 ? (
              <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
                ...
                <p className="text-ds-muted-foreground text-sm">
                  {t(locale, "socialCommerce.no_results_hint")}
                </p>
              </div>
            ) : (
                ...
                {filteredItems.map((item: any) => (
                  ...
                  >
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-destructive/10 to-fuchsia-100 relative overflow-hidden">
                      ...
                      {item.platform && (
                        <span
                          className={`absolute top-2 start-2 px-2.5 py-1 text-xs font-semibold text-white rounded-full capitalize ${platformColors[item.platform?.toLowerCase()] || "bg-ds-muted-foreground"}`}
                        >
                    ...
                    <div className="p-4">
                      ...
                      {item.handle && (
                        <p className="text-xs text-ds-destructive mt-0.5">
                          @{item.handle}
                        </p>
                      )}
                      {item.description && (
                      ...
                      <div className="flex justify-between items-center pt-3 mt-3 border-t border-ds-border">
                        ...
                        <span className="px-3 py-1.5 text-xs font-semibold text-white bg-ds-destructive rounded-lg group-hover:bg-fuchsia-600 transition-colors">
                          {t(locale, "socialCommerce.visit_shop")}
                        </span>
                      </div>
                    </div>
                  </a>
      ...
      <section className="py-16 bg-ds-card border-t border-ds-border">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-ds-foreground text-center mb-12">
            {t(locale, "socialCommerce.why_shop_social")}
          </h2>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "socialCommerce.step1_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "socialCommerce.step1_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-destructive text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "socialCommerce.step2_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "socialCommerce.step2_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-destructive text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "socialCommerce.step3_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "socialCommerce.step3_desc")}
              </p>
            </div>
          </div>
        </div>
