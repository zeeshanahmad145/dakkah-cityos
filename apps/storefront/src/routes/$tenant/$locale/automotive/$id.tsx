  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/automotive/${params.id}`,
        {
...
function AutomotiveDetailPage() {
  ...
  const handleScheduleTestDrive = async () => {
    ...
    try {
      const baseUrl = getServerBaseUrl()
      const resp = await fetch(`${baseUrl}/store/automotive/${id}`, {
        method: "POST",
        headers: {
        ...
        credentials: "include",
        body: JSON.stringify({ vehicle_id: id, type: "test_drive" }),
      })
      if (resp.ok) toast.success("Test drive scheduled!")
  ...
  const handleContactDealer = () => {
    toast.success("Contact request sent to dealer!")
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This vehicle listing may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/automotive` as never}
  ...
  const specs = [
    ...
    {
      label: t(locale, "automotive.label_model", "Model"),
      value: vehicle.model,
    },
    {
      label: t(locale, "automotive.label_mileage", "Mileage"),
      value: vehicle.mileage
        ? `${Number(vehicle.mileage || 0).toLocaleString()} mi`
        : null,
    },
    {
      label: t(locale, "automotive.label_fuel_type", "Fuel Type"),
      value: vehicle.fuel_type || vehicle.fuelType,
    },
    {
      label: t(locale, "automotive.label_transmission", "Transmission"),
      value: vehicle.transmission,
    },
    {
      label: t(locale, "automotive.label_color", "Color"),
      value: vehicle.color || vehicle.exterior_color,
    },
    { label: t(locale, "automotive.label_vin", "VIN"), value: vehicle.vin },
    {
      label: t(locale, "automotive.label_engine", "Engine"),
      value: vehicle.engine,
    },
    {
      label: t(locale, "automotive.label_drivetrain", "Drivetrain"),
      value: vehicle.drivetrain,
    },
    {
      label: t(locale, "automotive.label_body_type", "Body Type"),
      value: vehicle.body_type || vehicle.bodyType,
    },
    {
      label: t(locale, "automotive.label_doors", "Doors"),
      value: vehicle.doors,
    },
  ].filter((s) => s.value)
            ...
            {vehicle.features && (vehicle.features as string[]).length > 0 && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(vehicle.features as string[]).map((feature: string, idx: number) => (
                    <div
                      key={idx}
            ...
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                ...
                >
                  ...
                  {testDriveLoading ? "Scheduling..." : "Schedule Test Drive"}
                </button>
