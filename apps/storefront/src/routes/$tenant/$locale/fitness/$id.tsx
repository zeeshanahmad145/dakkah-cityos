  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/fitness/${params.id}`,
        {
  ...
  const handleBookClass = async () => {
    ...
    try {
      ...
      const resp = await fetch(
        `${baseUrl}/store/fitness/classes/${id}/check-in`,
        {
          method: "POST",
          headers: {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This fitness class or membership may have been removed or is no
              longer available.
            </p>
            <Link
              to={`${prefix}/fitness` as never}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              ...
              {item.membership_options &&
                item.membership_options.length > 0 && (
                  <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                      ...
                      {item.membership_options.map((opt: any, idx: number) => {
                        ...
                        return (
                          <button
                            key={idx}
                            onClick={() =>
                              handleSelectMembership(planId, opt.name)
                            }
                            disabled={isLoading}
                            className="w-full text-start p-3 border border-ds-border rounded-lg hover:border-ds-primary transition-colors cursor-pointer disabled:opacity-50"
                          >
                            <div className="flex justify-between items-center">
                              <p className="font-medium text-ds-foreground text-sm">
                                {opt.name}
                              </p>
                              <p className="font-bold text-ds-primary text-sm">
                ...
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-ds-primary/10 rounded-full flex items-center justify-center text-ds-primary font-semibold">
                      {(item.instructor_info.name || item.instructor || "I")
                        .charAt(0)
                        .toUpperCase()}
                    </div>
                    <div>
                      <p className="font-medium text-ds-foreground">
                        {item.instructor_info.name || item.instructor}
                      </p>
