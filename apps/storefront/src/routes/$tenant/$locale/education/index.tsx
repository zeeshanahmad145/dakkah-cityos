...
export const Route = createFileRoute("/$tenant/$locale/education/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse educational courses on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
    try {
      ...
      const items = raw.map((item: any) => {
        ...
        return {
          ...
          short_description:
            item.short_description || meta.short_description || "",
...
function EducationPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    ...
    const matchesLevel =
      levelFilter === "all" || item.level?.toLowerCase() === levelFilter
    const matchesFormat =
      formatFilter === "all" || item.format?.toLowerCase() === formatFilter
      ...
      <div className="bg-gradient-to-r from-ds-primary to-ds-primary/90 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "education.title")}
          </h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto">
            Expand your knowledge with expert-led courses, workshops, and
            programs across a wide range of topics.
          </p>
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            ...
            <span>
              {t(
                locale,
                "education.badge_expert_instructors",
                "Expert instructors",
              )}
            </span>
            <span>|</span>
            <span>
              {t(
                locale,
                "education.badge_flexible_learning",
                "Flexible learning",
              )}
            </span>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-72 flex-shrink-0">
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "verticals.level_label")}
                </label>
                <div className="space-y-1">
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "verticals.format_label")}
                </label>
                <div className="space-y-1">
                  {formatOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "verticals.all_formats")
                        : formatLabels[opt] ||
                          opt.charAt(0).toUpperCase() + opt.slice(1)}
                    </button>
                  ))}
                ...
                {filteredItems.map((item: any) => (
                  ...
                  >
                    <div className="aspect-video bg-gradient-to-br from-ds-primary/10 to-ds-primary/15 relative overflow-hidden">
                      ...
                      {item.level && (
                        <span
                          className={`absolute top-2 start-2 px-2 py-1 text-xs font-medium rounded-md capitalize ${levelColors[item.level?.toLowerCase()] || "bg-ds-muted text-ds-foreground/80"}`}
                        >
                          {item.level}
                        </span>
                      )}
                      {item.format && (
                        <span className="absolute top-2 end-2 px-2 py-1 text-xs font-medium bg-ds-card/90 text-ds-foreground/80 rounded-md">
                          {formatLabels[item.format?.toLowerCase()] ||
                            item.format}
                        </span>
                      )}
                    </div>
                    <div className="p-4">
                      <h3 className="font-semibold text-ds-foreground group-hover:text-ds-primary transition-colors line-clamp-2">
                        {item.title}
                      </h3>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Explore our catalog of expert-led courses across multiple
                disciplines.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Sign up for your chosen course and start learning at your own
                pace.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Complete the course and earn a certificate to showcase your
                skills.
              </p>
            </div>
          </div>
