  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/financial-products/${params.id}`,
        {
...
function FinancialDetailPage() {
  ...
  const handleApplyNow = async () => {
    setLoading(true)
    try {
      const resp = await fetch(`${baseUrl}/store/financial/applications`, {
        method: "POST",
        headers: {
      ...
      if (resp.ok) toast.success("Application submitted successfully!")
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This financial product may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/financial` as never}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {product.rate != null && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-4 text-center">
                  ...
                  <p className="text-lg font-bold text-ds-primary">
                    {product.rate}%
                  </p>
              ...
              {product.min_amount != null && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-4 text-center">
                  ...
                  <p className="text-lg font-bold text-ds-foreground">
                    ${Number(product.min_amount || 0).toLocaleString()}
                  </p>
                </div>
              )}
            ...
            {product.features && (product.features as string[]).length > 0 && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {(product.features as string[]).map((feature: any, idx: number) => (
                    <div
                      key={idx}
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="space-y-2">
                  {product.requirements.map((req: any, idx: number) => (
                    ...
                    >
                      <svg
                        className="w-4 h-4 text-ds-muted-foreground flex-shrink-0"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                  ...
                  {product.fees.map((fee: any, idx: number) => (
                    ...
                    >
                      <span className="text-sm text-ds-foreground">
                        {typeof fee === "string" ? fee : fee.name}
                      </span>
                      {fee.amount != null && (
