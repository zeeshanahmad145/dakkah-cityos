...
export const Route = createFileRoute("/$tenant/$locale/warranties/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse warranty options on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
    try {
      ...
      const items = raw.map((item: any) => {
        ...
        return {
          ...
          coverage: (() => {
            const c = item.coverage || meta.coverage
            if (!c) return null
            if (typeof c === "string") return [c]
            if (Array.isArray(c))
              return c.map((x: any) => (typeof x === "string" ? x : String(x)))
            return Object.keys(c).filter((k: string) => c[k])
          })(),
          exclusions: (() => {
            const e = item.exclusions || meta.exclusions
            if (!e) return null
            if (typeof e === "string") return [e]
            if (Array.isArray(e))
              return e.map((x: any) => (typeof x === "string" ? x : String(x)))
            return Object.keys(e).filter((k: string) => e[k])
          })(),
...
const planTypeColors: Record<string, { badge: string; accent: string }> = {
  basic: {
    badge: "bg-ds-muted text-ds-foreground/80",
    accent: "text-ds-muted-foreground",
  },
  standard: { badge: "bg-ds-info/15 text-ds-info", accent: "text-ds-primary" },
  premium: {
    badge: "bg-ds-warning/15 text-ds-warning",
    accent: "text-ds-warning",
  },
  extended: {
    badge: "bg-ds-success/15 text-ds-success",
...
function WarrantiesPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    ...
    const matchesPlanType =
      planTypeFilter === "all" ||
      item.plan_type?.toLowerCase() === planTypeFilter
      ...
      <div className="bg-gradient-to-r from-ds-success to-ds-success text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            ...
            <span>|</span>
            <span>
              {t(locale, "warranties.badge_full_coverage", "Full coverage")}
            </span>
            <span>|</span>
            <span>
              {t(locale, "warranties.badge_easy_claims", "Easy claims")}
            </span>
          </div>
        </div>
      </div>
                ...
                {filteredItems.map((item: any) => (
                  ...
                  >
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-success/10 to-ds-success/15 relative overflow-hidden">
                      ...
                      {item.plan_type && (
                        <span
                          className={`absolute top-2 start-2 px-2.5 py-1 text-xs font-semibold rounded-md capitalize ${planTypeColors[item.plan_type?.toLowerCase()]?.badge || "bg-ds-muted text-ds-foreground/80"}`}
                        >
                          {item.plan_type}
                        </span>
                      )}
                    ...
                    <div className="p-4">
                        ...
                        <div className="mt-3">
                          ...
                          <ul className="text-xs text-ds-muted-foreground space-y-1">
                            {(Array.isArray(item.coverage)
                              ? item.coverage.slice(0, 3)
                              : [item.coverage]
                            ).map((c: string, i: number) => (
                              <li key={i} className="flex items-center gap-1">
                                <svg
                                  className="w-3 h-3 text-ds-success flex-shrink-0"
                                  fill="none"
                                  viewBox="0 0 24 24"
                                  stroke="currentColor"
                                >
                      ...
                      <div className="flex justify-between items-center pt-3 mt-3 border-t border-ds-border">
                        ...
                        <span className="px-3 py-1.5 text-xs font-semibold text-white bg-ds-success rounded-lg group-hover:bg-ds-success/90 transition-colors">
                          {t(locale, "warranty.get_protection")}
                        </span>
                      </div>
                    </div>
                  </a>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Choose from plans that cover mechanical failures, accidental
                damage, and more.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                File a claim online in minutes and get fast resolution from our
                team.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Rest easy knowing your valuable purchases are protected
                long-term.
              </p>
            </div>
          </div>
