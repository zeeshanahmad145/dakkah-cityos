  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/parking/${params.id}`,
        {
...
function ParkingDetailPage() {
  ...
  const handleReserveSpot = async () => {
    ...
    try {
      const baseUrl = getServerBaseUrl()
      const resp = await fetch(`${baseUrl}/store/parking/enter`, {
        method: "POST",
        headers: {
      ...
      if (resp.ok) toast.success("Spot reserved!")
  ...
  const handleGetDirections = () => {
    ...
    const address =
      typeof location === "string"
        ? location
        : location?.address || spot?.address || spot?.name
    if (address) {
      window.open(
      ...
      toast.info("Directions are not available for this location.")
    }
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This parking spot may have been removed or is no longer available.
            </p>
            <Link
              to={`${prefix}/parking` as never}
  ...
  return (
    <div className="min-h-screen bg-ds-background">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              ...
              {spot.available != null && (
                <span
                  className={`absolute top-4 end-4 px-3 py-1 text-xs font-semibold rounded-full ${spot.available ? "bg-ds-success/20 text-ds-success" : "bg-ds-destructive/20 text-ds-destructive"}`}
                >
            ...
            <div>
              ...
              {spot.location && (
                <div className="flex items-start gap-2 mt-2 text-sm text-ds-muted-foreground">
                  ...
                  <span>
                    {typeof spot.location === "string"
                      ? spot.location
                      : spot.location.address || spot.address}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              {spot.daily_rate != null && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-4 text-center">
                  ...
                  <p className="text-lg font-bold text-ds-foreground">
                    ${Number(spot.daily_rate).toFixed(2)}
                  </p>
                </div>
              )}
              {spot.monthly_rate != null && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-4 text-center">
                  ...
                  <p className="text-lg font-bold text-ds-foreground">
                    ${Number(spot.monthly_rate || 0).toLocaleString()}
                  </p>
                </div>
              )}
            ...
            {spot.features && (spot.features as string[]).length > 0 && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                <h2 className="font-semibold text-ds-foreground mb-3">
                  Features & Amenities
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {(spot.features as string[]).map((feature: any, idx: number) => (
                    <div
                      key={idx}
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-6">
              ...
              <div className="aspect-[16/9] bg-ds-muted rounded-lg flex items-center justify-center">
                <div className="text-center">
                  <svg
                    className="w-12 h-12 text-ds-muted-foreground/30 mx-auto mb-2"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
            ...
            {spot.operating_hours && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                {Array.isArray(spot.operating_hours) ? (
                    ...
                    {spot.operating_hours.map((hours: any, idx: number) => (
                      ...
                      >
                        <span className="text-ds-foreground font-medium">
                          {typeof hours === "string" ? hours : hours.day}
                        </span>
                        {hours.hours && (
                  ...
                  <p className="text-sm text-ds-muted-foreground whitespace-pre-wrap">
                    {spot.operating_hours}
                  </p>
                )}
              </div>
            ...
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                ...
                >
                  ...
                  {reserveLoading ? "Reserving..." : "Reserve Spot"}
                </button>
                ...
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  ...
                  <div className="space-y-2">
                    {spot.rates.map((rate: any, idx: number) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-3 bg-ds-muted/30 rounded-lg"
                      >
                ...
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  ...
                  <div className="space-y-2">
                    {spot.rules.map((rule: any, idx: number) => (
                      ...
                      >
                        <svg
                          className="w-4 h-4 text-ds-warning flex-shrink-0"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {(spot.latitude ||
          spot.lat ||
          (spot.coordinates && spot.coordinates.lat)) && <MapBlock />}
