...
export const Route = createFileRoute("/$tenant/$locale/parking/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse parking options on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
    try {
      ...
      const resp = await fetchWithTimeout(`${baseUrl}/store/parking/spots`, {
        headers: {
      ...
      const items = raw.map((s: any) => {
        ...
        return {
          ...
          address:
            typeof (s.address || meta.address) === "string"
              ? s.address || meta.address
              : s.address || meta.address
                ? [
                    s.address?.line1 || meta.address?.line1,
                    s.address?.city || meta.address?.city,
                    s.address?.country || meta.address?.country,
                  ]
                    .filter(Boolean)
                    .join(", ")
                : null,
          ...
          operating_hours:
            typeof (s.operating_hours || meta.operating_hours) === "string"
              ? s.operating_hours || meta.operating_hours
              : s.operating_hours || meta.operating_hours
                ? JSON.stringify(s.operating_hours || meta.operating_hours)
                    .replace(/[{}"]/g, "")
                    .replace(/,/g, ", ")
                : null,
        }
...
function ParkingPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    const matchesSearch = searchQuery
      ...
      : true
    const matchesZone =
      zoneTypeFilter === "all" || item.zone_type === zoneTypeFilter
  ...
  const zoneLabel = (z: string) => {
    ...
    return (
      map[z] || z.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase())
    )
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm text-white/70 mb-4">
            ...
            <span className="text-white">
              {t(locale, "parking.breadcrumb")}
            </span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "parking.hero_title")}
          </h1>
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            <span>
              {items.length} {t(locale, "parking.facilities_count")}
            </span>
            <span>|</span>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-72 flex-shrink-0">
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "parking.zone_type_label")}
                </label>
                <div className="space-y-1">
                  {zoneTypeOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "verticals.all_zone_types")
                        : zoneLabel(opt)}
                    </button>
                  ))}
                </div>
            ...
            {filteredItems.length === 0 ? (
              <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
                ...
                <p className="text-ds-muted-foreground text-sm">
                  {t(locale, "parking.no_results_hint")}
                </p>
              </div>
            ) : (
                ...
                {filteredItems.map((item: any) => (
                  ...
                  >
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-primary/10 to-ds-primary/15 relative overflow-hidden">
                      ...
                      {item.zone_type && (
                        <span className="absolute top-2 start-2 px-2 py-1 text-xs font-medium bg-ds-primary text-white rounded-md">
                          {zoneIcon(item.zone_type)} {zoneLabel(item.zone_type)}
                        </span>
                      )}
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center gap-3 mt-3 text-xs text-ds-muted-foreground">
                        {item.available_spots != null && (
                          <span
                            className={`flex items-center gap-1 font-medium ${availabilityColor(item.available_spots, item.total_spots)}`}
                          >
                            <svg
                            ...
                            {item.available_spots === 0
                              ? "Full"
                              : `${item.available_spots}${item.total_spots ? `/${item.total_spots}` : ""} spots`}
                          </span>
                        )}
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "parking.step1_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "parking.step1_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-primary text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "parking.step2_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "parking.step2_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-primary text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "parking.step3_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "parking.step3_desc")}
              </p>
            </div>
          </div>
        </div>
