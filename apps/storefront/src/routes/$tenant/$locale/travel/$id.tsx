  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/travel/${params.id}`,
        {
...
function TravelDetailPage() {
  ...
  const handleBookNow = async () => {
    ...
    try {
      const resp = await fetch(`${baseUrl}/store/travel/${id}`, {
        method: "POST",
        headers: {
      ...
      if (resp.ok) toast.success("Booking request submitted!")
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This travel package may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/travel` as never}
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="space-y-4">
                  {pkg.itinerary.map((day: any, idx: number) => (
                    <div
                      key={idx}
                      className="relative ps-6 pb-4 border-l-2 border-ds-primary/30 last:pb-0"
                    >
                      <div className="absolute -start-[9px] top-0 w-4 h-4 bg-ds-primary rounded-full flex items-center justify-center">
                        <span className="text-[10px] text-ds-primary-foreground font-bold">
                          {idx + 1}
                        </span>
                      </div>
                      <h3 className="font-medium text-ds-foreground text-sm">
                        {day.title || day.name || `Day ${idx + 1}`}
                      </h3>
                      {day.description && (
                        ...
                        <ul className="mt-2 space-y-1">
                          {day.activities.map(
                            (activity: string, aIdx: number) => (
                              <li
                                key={aIdx}
                                className="text-sm text-ds-muted-foreground flex items-center gap-2"
                              >
                                <svg
                                  className="w-3 h-3 text-ds-primary flex-shrink-0"
                                  fill="currentColor"
                                  viewBox="0 0 8 8"
                                >
            ...
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                ...
                >
                  ...
                  {inquireLoading ? "Sending..." : "Inquire"}
                </button>
              </div>
                      ...
                      >
                        ...
                        {date.price && (
                          <span className="text-sm font-medium text-ds-primary">
                            ${Number(date.price || 0).toLocaleString()}
                          </span>
                        )}
                      </div>
                    ))}
