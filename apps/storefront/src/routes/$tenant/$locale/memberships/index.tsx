...
export const Route = createFileRoute("/$tenant/$locale/memberships/")({
  ...
  loader: async () => {
    try {
      ...
      const raw =
        data.memberships || data.tiers || data.plans || data.items || []
      const items = raw.map((m: any) => {
        ...
        return {
          ...
          thumbnail:
            m.thumbnail || meta.thumbnail || meta.image || m.logo_url || null,
...
function MembershipsPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    const matchesSearch = searchQuery
      ...
      : true
    const matchesTier =
      tierFilter === "all" || (item.tier || "").toLowerCase() === tierFilter
      ...
      <div className="bg-gradient-to-r from-ds-warning to-ds-warning text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "memberships.title")}
          </h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto">
            Unlock exclusive benefits, discounts, and premium access with our
            membership tiers.
          </p>
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            ...
            <span>|</span>
            <span>
              {t(locale, "memberships.badge_cancel_anytime", "Cancel anytime")}
            </span>
            <span>|</span>
            <span>
              {t(
                locale,
                "memberships.badge_exclusive_perks",
                "Exclusive perks",
              )}
            </span>
          </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {tierOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "verticals.all_tiers")
                        : `${tierIcons[opt] || ""} ${opt.charAt(0).toUpperCase() + opt.slice(1)}`}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {filteredItems.length === 0 ? (
              <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
                ...
                <h3 className="text-lg font-semibold text-ds-foreground mb-2">
                  {t(locale, "memberships.no_results")}
                </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredItems.map((item: any) => {
                  ...
                  const gradient =
                    tierColors[tierKey] || "from-ds-warning to-ds-warning"
                  return (
                    <div
                      key={item.id}
                    ...
                    >
                      <div
                        className={`bg-gradient-to-br ${gradient} p-6 text-white text-center relative`}
                      >
                        {item.is_popular && (
                          <span className="absolute top-2 end-2 px-2 py-1 text-xs font-bold bg-ds-card text-ds-warning rounded-full">
                            Popular
                          </span>
                        )}
                        ...
                        {item.tier && (
                          <span className="inline-block mt-1 px-2 py-0.5 text-xs font-medium bg-ds-card/20 rounded-full capitalize">
                            {item.tier}
                          </span>
                        )}
                      </div>
                          ...
                          <ul className="space-y-2 mb-4">
                            {item.benefits
                              ...
                              .map((b: string, i: number) => (
                                <li
                                  key={i}
                                  className="flex items-start gap-2 text-sm text-ds-foreground"
                                >
                                  <svg
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Access members-only discounts, early releases, and special
                events.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                The more you engage, the higher your tier — unlock bigger
                benefits.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Upgrade, downgrade, or cancel your membership anytime with no
                hassle.
              </p>
            </div>
          </div>
