        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This B2B listing may have been removed or is no longer available.
            </p>
            <Link
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              {item.thumbnail || item.image || item.logo ? (
                <img
                  loading="lazy"
                  src={item.thumbnail || item.image || item.logo}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {item.products.map((product: any, idx: number) => (
                    ...
                    >
                      ...
                      {product.price && (
                        <p className="text-ds-primary font-semibold mt-1">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-ds-border">
                        <th className="text-left py-2 text-ds-muted-foreground font-medium">
                          Quantity
                        </th>
                        <th className="text-left py-2 text-ds-muted-foreground font-medium">
                          Price per Unit
                    ...
                    <tbody>
                      {item.bulk_pricing.map((tier: any, idx: number) => (
                        ...
                        >
                          <td className="py-2 text-ds-foreground">
                            {tier.min_quantity}
                            {tier.max_quantity ? `-${tier.max_quantity}` : "+"}
                          </td>
                          <td className="py-2 text-ds-foreground font-medium">
                            ${Number(tier.price || 0).toLocaleString()}
                          </td>
                          <td className="py-2 text-ds-success">
                            {tier.discount || "-"}
                          </td>
                        </tr>
                      ))}
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {item.certifications.map((cert: string, idx: number) => (
                    <span
                      key={idx}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-full bg-ds-info/10 text-ds-info border border-ds-info/30"
                    >
                      <svg
