  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/affiliate/${params.id}`,
        {
...
function AffiliateDetailPage() {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This affiliate program may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/affiliate` as never}
  ...
  const details = [
    {
      label: t(locale, "affiliate.label_commission_rate", "Commission Rate"),
      value: program.commission_rate
    ...
    {
      label: t(locale, "affiliate.label_cookie_duration", "Cookie Duration"),
      value:
        program.cookie_duration || program.cookieDuration
          ? `${program.cookie_duration || program.cookieDuration} days`
          : null,
    },
    {
      label: t(locale, "affiliate.label_payout_schedule", "Payout Schedule"),
      value: program.payout_schedule || program.payoutSchedule,
    },
    {
      label: t(locale, "affiliate.label_minimum_payout", "Minimum Payout"),
      value:
        program.minimum_payout != null
          ? `$${Number(program.minimum_payout || 0).toLocaleString()}`
          : program.minimumPayout != null
            ? `$${Number(program.minimumPayout || 0).toLocaleString()}`
            : null,
    },
    {
      label: t(locale, "affiliate.label_payment_method", "Payment Method"),
      value: program.payment_method || program.paymentMethod,
    },
    {
      label: t(locale, "affiliate.label_status", "Status"),
  ...
  return (
    <div className="min-h-screen bg-ds-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {program.thumbnail && (
          <div className="mb-8 rounded-xl overflow-hidden aspect-[21/9]">
            <img
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <ul className="space-y-3 text-sm text-ds-muted-foreground">
                  <li className="flex items-start gap-2">
                    <span className="w-5 h-5 bg-ds-primary/10 text-ds-primary rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">
                      1
                    </span>
