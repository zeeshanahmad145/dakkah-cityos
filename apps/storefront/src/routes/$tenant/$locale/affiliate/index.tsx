...
export const Route = createFileRoute("/$tenant/$locale/affiliate/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse affiliate programs on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
      ...
      const tiers = [
        {
          ...
          name: "Starter",
          commission: "5%",
          requirement: "0+ sales/month",
          earnings: "Up to 2,500 SAR/mo",
          color: "gray",
          features: [
            "Basic dashboard",
            "Standard links",
        ...
        },
        {
          ...
          name: "Pro",
          commission: "10%",
          requirement: "20+ sales/month",
          earnings: "Up to 10,000 SAR/mo",
          color: "blue",
          features: [
            "Advanced analytics",
            "Custom links",
        ...
        },
        {
          ...
          name: "Elite",
          commission: "15%",
          requirement: "50+ sales/month",
          earnings: "Up to 30,000 SAR/mo",
          color: "purple",
          features: [
            "Real-time analytics",
            "API access",
...
function AffiliatePage() {
  ...
  const colorBorder: Record<string, string> = {
    gray: "border-ds-border",
    blue: "border-ds-info ring-2 ring-ds-primary/20",
    purple: "border-ds-primary ring-2 ring-ds-primary/20",
    amber: "border-ds-warning",
  }
  const colorBg: Record<string, string> = {
    gray: "from-ds-muted-foreground to-ds-muted-foreground/80",
  ...
  const colorBtn: Record<string, string> = {
    gray: "bg-ds-muted-foreground hover:bg-ds-foreground/80",
    blue: "bg-ds-primary hover:bg-ds-primary/90",
    purple: "bg-ds-primary hover:bg-ds-primary/90",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm text-white/70 mb-4">
            ...
            <span className="text-white">
              {t(locale, "affiliate.breadcrumb")}
            </span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "affiliate.hero_title")}
          </h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto">
            {t(locale, "affiliate.hero_subtitle")}
          </p>
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            <span>{t(locale, "affiliate.badge_commission")}</span>
            <span>|</span>
            <span>{t(locale, "affiliate.badge_tracking")}</span>
            <span>|</span>
            <span>{t(locale, "affiliate.badge_payouts")}</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          <input
            ...
            placeholder={t(locale, "affiliate.search_placeholder")}
        ...
        <h2 className="text-2xl font-bold text-ds-foreground mb-6">
          {t(locale, "affiliate.commission_tiers")}
        </h2>
        {filteredTiers.length === 0 ? (
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground text-sm">
              {t(locale, "affiliate.no_results_hint")}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {filteredTiers.map((tier: any) => (
              <div
                key={tier.id}
                className={`relative bg-ds-background border ${tier.popular ? colorBorder[tier.color] : "border-ds-border"} rounded-xl p-6 hover:shadow-lg transition-all duration-200`}
              >
                {tier.popular && (
                  <span className="absolute -top-3 start-6 px-3 py-1 text-xs font-bold bg-ds-primary text-white rounded-full">
                    {t(locale, "affiliate.best_value")}
                  </span>
                )}
                ...
                <div className="flex items-baseline gap-1 mb-2">
                  ...
                  <span className="text-sm text-ds-muted-foreground">
                    {t(locale, "affiliate.commission_label")}
                  </span>
                </div>
                <p className="text-xs text-ds-muted-foreground mb-1">
                  {tier.requirement}
                </p>
                <p className="text-sm font-medium text-ds-foreground mb-4">
                  {tier.earnings}
                </p>
                ...
                <button
                  className={`w-full py-2.5 rounded-lg font-medium text-white transition-colors ${colorBtn[tier.color]}`}
                >
                  {t(locale, "affiliate.join_now")}
                </button>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {[
              {
                step: "1",
                title: t(locale, "affiliate.title1_sign_up", "Sign Up"),
                desc: t(
                  locale,
                  "affiliate.desc5_create_your_free_aff",
                  "Create your free affiliate account in minutes.",
                ),
              },
              {
                step: "2",
                title: t(locale, "affiliate.title2_get_links", "Get Links"),
                desc: t(
                  locale,
                  "affiliate.desc6_generate_unique_refe",
                  "Generate unique referral links for any product.",
                ),
              },
              {
                step: "3",
                title: t(locale, "affiliate.title3_promote", "Promote"),
                desc: t(
                  locale,
                  "affiliate.desc7_share_links_on_your",
                  "Share links on your website, social media, or blog.",
                ),
              },
              {
                step: "4",
                title: t(locale, "affiliate.title4_earn", "Earn"),
                desc: t(
                  locale,
                  "affiliate.desc8_receive_commissions",
                  "Receive commissions for every successful sale.",
                ),
              },
            ].map((s) => (
