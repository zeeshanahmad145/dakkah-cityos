...
export const Route = createFileRoute("/$tenant/$locale/charity/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse charitable causes on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
    try {
      ...
      const items = raw.map((c: any) => {
        ...
        return {
          id: c.id,
          title:
            meta.title || c.title || c.name || meta.name || "Untitled Campaign",
          ...
          raised:
            meta.raised || c.raised || c.amount_raised || c.current_amount || 0,
          donor_count: meta.donor_count || c.donor_count || c.backers || 0,
          organization:
            meta.organization || c.organization || c.organizer || null,
...
function CharityPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    ...
    const matchesType =
      typeFilter === "all" ||
      (item.campaign_type || "").toLowerCase() === typeFilter
      ...
      <div className="bg-gradient-to-r from-ds-success to-ds-success text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "charity.title")}
          </h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto">
            Make a difference — support causes that matter to you and help
            change lives.
          </p>
                  ...
                  return (
                    ...
                    >
                      <div className="aspect-[4/3] bg-gradient-to-br from-ds-success/10 to-ds-success/15 relative overflow-hidden">
                        ...
                        {item.campaign_type && (
                          <span
                            className={`absolute top-2 start-2 px-2 py-1 text-xs font-medium text-white rounded-md ${typeColors[item.campaign_type] || "bg-ds-success"}`}
                          >
                            {typeLabels[item.campaign_type] ||
                              item.campaign_type}
                          </span>
                        )}
                        {item.is_urgent && (
                          <span className="absolute top-2 end-2 px-2 py-1 text-xs font-bold bg-ds-destructive text-white rounded-md">
                            Urgent
                          </span>
                        )}
                      </div>
                      <div className="p-4">
                        ...
                        <div className="mt-3">
                          <div className="w-full bg-ds-muted rounded-full h-2.5 overflow-hidden">
                            <div
                              className="h-full bg-ds-success rounded-full transition-all duration-500"
                              style={{ width: `${progress}%` }}
                            />
                          </div>
                          <div className="flex justify-between mt-1.5 text-xs text-ds-muted-foreground">
                            <span className="font-medium text-ds-success">
                              {formatAmount(raised, item.currency)} raised
                            </span>
                            <span>
                              {formatAmount(goal, item.currency)} goal
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="pt-3 mt-3 border-t border-ds-border flex justify-between items-center">
                          ...
                          <span className="px-3 py-1.5 text-xs font-semibold text-white bg-ds-success rounded-lg group-hover:bg-ds-success transition-colors">
                            {t(locale, "charity.donate_now")}
                          </span>
                        </div>
                      </div>
                    </a>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Track exactly how funds are used with real-time progress
                updates.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Join thousands of donors making a difference together.
              </p>
            </div>
          </div>
        </div>
