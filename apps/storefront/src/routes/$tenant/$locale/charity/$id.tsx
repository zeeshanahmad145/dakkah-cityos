  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/charity/${params.id}`,
        {
...
function CharityDetailPage() {
  ...
  const handleShare = async () => {
    const shareData = {
      title: campaign?.name || campaign?.title || "Charity Campaign",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This charity campaign may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/charity` as never}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-6">
              <div className="flex items-end justify-between mb-2">
                <div>
                  <span className="text-2xl font-bold text-ds-primary">
                    ${Number(raised || 0).toLocaleString()}
                  </span>
                  <span className="text-sm text-ds-muted-foreground ms-1">
                    raised of ${Number(goal || 0).toLocaleString()} goal
                  </span>
                </div>
              ...
              <div className="w-full bg-ds-muted rounded-full h-3 overflow-hidden">
                <div
                  className="bg-ds-primary h-full rounded-full transition-all duration-500"
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="space-y-4">
                  {campaign.updates.map((update: any, idx: number) => (
                    <div
                      key={idx}
                      className="relative ps-6 pb-4 border-l-2 border-ds-border last:pb-0"
                    >
                      ...
                      <p className="text-sm text-ds-muted-foreground mt-1">
                        {update.content || update.text || update.description}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="space-y-3">
                  {campaign.donors.map((donor: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between py-2 border-b border-ds-border last:border-0"
                    >
                      ...
                      {donor.amount != null && (
                        <span className="font-medium text-ds-primary text-sm">
            ...
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                ...
                <div className="flex gap-2">
                  {[10, 25, 50, 100].map((amt) => (
                    <button
                      ...
                      className={`flex-1 py-1.5 text-xs font-medium rounded-lg border transition-colors ${donationAmount === String(amt) ? "bg-ds-primary text-ds-primary-foreground border-ds-primary" : "border-ds-border text-ds-muted-foreground hover:bg-ds-muted"}`}
                    >
                      ${amt}
                    </button>
                  ))}
                ...
                <button
                  ...
                  disabled={donating}
                  className="w-full py-3 px-4 bg-ds-primary text-ds-primary-foreground rounded-lg font-medium hover:bg-ds-primary/90 transition-colors flex items-center justify-center gap-2 text-lg disabled:opacity-50"
                >
                  <svg
                    className="w-5 h-5"
                    fill="none"
