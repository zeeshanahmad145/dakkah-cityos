...
export const Route = createFileRoute("/$tenant/$locale/bookings/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse and book services on Dakkah CityOS",
      },
    ],
  }),
  loader: async ({ abortController }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/bookings/services`,
        {
          headers: {
                    ...
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-primary/10 to-ds-primary/15 relative overflow-hidden">
                      ...
                      {item.location_type && (
                        <span className="absolute top-2 end-2 px-2 py-1 text-xs font-medium bg-ds-card/90 text-ds-foreground/80 rounded-md capitalize">
                          {item.location_type === "in_person"
                            ? "In Person"
                            : item.location_type === "customer_location"
                              ? "At Your Location"
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Browse available services and select the one that fits your
                needs.
              </p>
            </div>
            <div className="text-center">
