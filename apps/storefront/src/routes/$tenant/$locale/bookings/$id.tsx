...
function normalizeDetail(item: any) {
  ...
  return {
    ...
    service_name:
      meta.name || item.service_name || item.product_id || "Booking",
  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/bookings/${params.id}`,
        {
          headers: {
...
function BookingDetailPage() {
  ...
  const handleCancel = async () => {
    ...
    try {
      const baseUrl = getServerBaseUrl()
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/bookings/${id}/cancel`,
        {
          method: "POST",
          headers: {
            "x-publishable-api-key": getMedusaPublishableKey(),
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div>
              <div className="flex items-start justify-between gap-4">
                ...
                <span
                  className={`px-3 py-1 text-xs font-semibold rounded-full whitespace-nowrap ${statusStyles[displayStatus] || statusStyles.pending}`}
                >
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-ds-foreground font-mono text-xs">
                    {booking.id}
                  </span>
                </div>
                {booking.start_time && (
            ...
            {booking.notes && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <p className="text-ds-muted-foreground text-sm whitespace-pre-wrap">
                  {booking.notes}
                </p>
              </div>
            )}
