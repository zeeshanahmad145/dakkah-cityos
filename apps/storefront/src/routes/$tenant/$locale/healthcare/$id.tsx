...
function HealthcareDetailPage() {
  ...
  const handleBookAppointment = async () => {
    setLoading(true)
    try {
      const resp = await fetch(`${baseUrl}/store/healthcare/appointments`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-publishable-api-key": publishableKey },
        credentials: "include",
        body: JSON.stringify({ provider_id: id })
      })
      if (resp.ok) toast.success("Appointment booked successfully!")
  ...
  const handleCallOffice = () => {
    if (provider?.phone) {
      ...
      toast.info("Phone number not available. Please try booking online.")
    }
  }

  const handleVirtualVisit = () => {
    toast.success("Virtual visit request submitted! You will receive a link shortly.")
  }
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                <button onClick={handleBookAppointment} disabled={loading} className="w-full py-3 px-4 bg-ds-primary text-ds-primary-foreground rounded-lg font-medium hover:bg-ds-primary/90 transition-colors flex items-center justify-center gap-2 disabled:opacity-50">
                ...
                <button onClick={handleCallOffice} className="w-full py-3 px-4 border border-ds-border text-ds-foreground rounded-lg font-medium hover:bg-ds-muted transition-colors flex items-center justify-center gap-2">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                  Call Office
                </button>

                <button onClick={handleVirtualVisit} className="w-full py-3 px-4 border border-ds-border text-ds-foreground rounded-lg font-medium hover:bg-ds-muted transition-colors flex items-center justify-center gap-2">
