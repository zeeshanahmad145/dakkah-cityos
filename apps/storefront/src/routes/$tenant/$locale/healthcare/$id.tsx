  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/healthcare/${params.id}`,
        {
...
function HealthcareDetailPage() {
  ...
  const handleVirtualVisit = () => {
    toast.success(
      "Virtual visit request submitted! You will receive a link shortly.",
    )
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This healthcare provider may have been removed or is no longer
              available.
            </p>
            <Link
              ...
              <div className="flex items-start gap-4">
                <div className="w-20 h-20 bg-ds-primary/10 rounded-xl flex items-center justify-center text-ds-primary flex-shrink-0">
                  {provider.thumbnail || provider.image ? (
                    <img
                      loading="lazy"
                      src={provider.thumbnail || provider.image}
                      alt={provider.name}
                ...
                <div className="flex-1">
                  ...
                  {provider.specialty && (
                    <p className="text-ds-primary font-medium mt-1">
                      {provider.specialty}
                    </p>
                  )}
              ...
              provider.insurance_accepted.length > 0 && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {provider.insurance_accepted.map(
                ...
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  ...
                  <div className="space-y-2">
                    {provider.availability.map((slot: any, idx: number) => (
                      <button
                        key={idx}
                        className="w-full text-start px-3 py-2 text-sm rounded-lg border border-ds-border hover:border-ds-primary hover:bg-ds-primary/5 transition-colors"
                      >
                        <p className="font-medium text-ds-foreground">
                          {slot.date || slot.day}
                        </p>
                        <p className="text-xs text-ds-muted-foreground">
                          {slot.time || slot.hours}
                        </p>
                      </button>
                    ))}
                  </div>
                ...
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  ...
                  <div className="space-y-2 text-sm">
                    {(Array.isArray(provider.credentials)
                      ...
                      <div
                        key={idx}
                        className="flex items-center gap-2 text-ds-muted-foreground"
                      >
                        <svg
                          className="w-4 h-4 text-ds-primary flex-shrink-0"
                          fill="none"
                        ...
                        <span>
                          {typeof cred === "string" ? cred : cred.name}
                        </span>
                      </div>
                    ))}
                  </div>
