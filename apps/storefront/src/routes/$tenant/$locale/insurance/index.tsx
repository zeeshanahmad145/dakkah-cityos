...
export const Route = createFileRoute("/$tenant/$locale/insurance/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse insurance products on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
    try {
      ...
      const items = raw.map((item: any) => {
        ...
        return {
          ...
          coverage_details:
            item.coverage_details || meta.coverage_details || null,
...
function InsurancePage() {
  ...
  const filteredItems = items.filter((item: any) => {
    const matchesSearch = searchQuery
      ...
      : true
    const matchesType =
      typeFilter === "all" || item.insurance_type === typeFilter
      ...
      <div className="bg-gradient-to-r from-ds-info to-ds-success/90 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "insurance.title")}
          </h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto">
            Protect what matters most with comprehensive coverage options
                  ...
                  {typeOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ...
                        : `${typeIcon(opt)} ${opt.charAt(0).toUpperCase() + opt.slice(1)}`}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {filteredItems.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredItems.map((item: any) => {
                  const highlights = getCoverageHighlights(
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-4">
                        <h3 className="font-semibold text-ds-foreground group-hover:text-ds-info transition-colors line-clamp-1">
                          {item.name}
                        </h3>
                        {item.description && (
                        ...
                        {highlights.length > 0 && (
                          <div className="mt-3 space-y-1">
                            {highlights.map((h: string, i: number) => (
                              <div
                                key={i}
                                className="flex items-center gap-2 text-xs text-ds-muted-foreground"
                              >
                                <svg
                                  className="w-3.5 h-3.5 text-ds-info flex-shrink-0"
                                  fill="none"
                                  viewBox="0 0 24 24"
                                  stroke="currentColor"
                                >
                        ...
                        <div className="flex justify-between items-center pt-3 mt-3 border-t border-ds-border">
                          ...
                          <span className="px-3 py-1.5 text-xs font-semibold text-white bg-ds-info rounded-lg group-hover:bg-ds-info/90 transition-colors">
                            {t(locale, "insurance.get_quote")}
                          </span>
                        </div>
                      </div>
                    </a>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-info text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
                1
              </div>
              ...
              <p className="text-sm text-ds-muted-foreground">
                Browse coverage options and compare plans side by side.
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-info text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Receive personalized quotes based on your coverage needs.
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-info text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Enroll in your chosen plan and enjoy peace of mind.
              </p>
            </div>
          </div>
        </div>
