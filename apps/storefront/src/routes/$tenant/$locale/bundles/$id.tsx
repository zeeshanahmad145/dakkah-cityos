  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/bundles/${params.id}`,
        {
...
function BundleDetailPage() {
  ...
  const totalValue =
    ...
    items.reduce((sum: number, i: any) => sum + (Number(i.price) || 0), 0)
  const bundlePrice = bundle.price || bundle.bundle_price
  const savingsAmount =
    totalValue && bundlePrice ? Number(totalValue) - Number(bundlePrice) : 0
  const savingsPercent =
    totalValue && bundlePrice
      ? ((savingsAmount / Number(totalValue)) * 100).toFixed(0)
      : 0
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              {bundle.thumbnail || bundle.image ? (
                <img
                  loading="lazy"
                  src={bundle.thumbnail || bundle.image}
                ...
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-ds-primary/10 to-ds-primary/15">
                  <svg
                    className="w-16 h-16 text-ds-primary"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
              ...
              <div className="flex flex-wrap items-center gap-4 mt-3">
                {bundlePrice != null && (
                  <span className="text-2xl font-bold text-ds-primary">
                    ${Number(bundlePrice || 0).toLocaleString()}
                  </span>
                )}
                {totalValue > 0 &&
                  bundlePrice &&
                  Number(totalValue) > Number(bundlePrice) && (
                    <span className="text-lg text-ds-muted-foreground line-through">
                      ${Number(totalValue || 0).toLocaleString()}
                    </span>
                  )}
                {Number(savingsPercent) > 0 && (
                  <span className="px-3 py-1 text-xs font-medium rounded-full bg-ds-success/15 text-ds-success">
                    {savingsPercent}% off
                  </span>
                )}
              </div>
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="space-y-3">
                  {items.map((item: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-center gap-4 p-3 bg-ds-muted/30 rounded-lg"
                    >
                      {item.thumbnail || item.image ? (
                        ...
                        <div className="w-16 h-16 bg-ds-muted rounded-lg flex items-center justify-center flex-shrink-0">
                          <svg
                            className="w-6 h-6 text-ds-muted-foreground/30"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                          >
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium text-ds-foreground text-sm truncate">
                          {item.title || item.name}
                        </h3>
                        {item.description && (
                          <p className="text-xs text-ds-muted-foreground mt-0.5 line-clamp-1">
                            {item.description}
                          </p>
                        )}
                      ...
                      {item.price != null && (
                        <span className="text-sm font-medium text-ds-foreground flex-shrink-0">
                    ...
                    Number(totalValue) > Number(bundlePrice) && (
                      <div className="mt-2">
                        ...
                        <p className="text-sm font-medium text-ds-success">
                          You save $
                          {Number(savingsAmount || 0).toLocaleString()}
                        </p>
                      </div>
                    )}
                </div>
