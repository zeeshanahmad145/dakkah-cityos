  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/credit/${params.id}`,
        {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This financing option may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/credit` as never}
            ...
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              {product.thumbnail || product.image ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-ds-success/10 to-ds-success/15">
                  <svg
                    className="w-16 h-16 text-ds-success"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {terms.map((term: any, idx: number) => {
                    const months =
                      typeof term === "object"
                        ? term.months || term.duration
                        : term
                    const termApr =
                      typeof term === "object" ? term.apr || apr : apr
                    const monthly =
                      price && months
                        ? calculateMonthly(
                            Number(price),
                    ...
                    return (
                      ...
                      >
                        <p className="font-bold text-ds-foreground">
                          {months} months
                        </p>
                        {monthly && (
