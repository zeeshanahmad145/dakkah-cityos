...
export const Route = createFileRoute("/$tenant/$locale/auctions/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse and bid on auctions on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
    try {
      ...
      const raw =
        data.items || data.auctions || data.listings || data.products || []
      ...
      <div className="bg-gradient-to-r from-ds-primary to-ds-destructive text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "auctions.title")}
          </h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto">
            Bid on unique items and discover incredible deals in real-time
            auctions.
          </p>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-72 flex-shrink-0">
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "verticals.status_label")}
                </label>
                <div className="space-y-1">
                  {statusOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "verticals.all_statuses")
                        : opt.charAt(0).toUpperCase() + opt.slice(1)}
                    </button>
                  ))}
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="aspect-[4/3] bg-gradient-to-br from-ds-primary/10 to-ds-destructive/15 relative overflow-hidden">
                        ...
                        {item.status && (
                          <span
                            className={`absolute top-2 end-2 px-2 py-1 text-xs font-medium rounded-md ${item.status === "active" ? "bg-ds-success text-white" : item.status === "upcoming" ? "bg-ds-info text-white" : "bg-ds-muted-foreground text-white"}`}
                          >
                      ...
                      <div className="p-4">
                        ...
                        {timeLeft && (
                          <div className="flex items-center gap-1 mt-2 text-xs">
                            <svg
                              className="w-3.5 h-3.5 text-ds-muted-foreground"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                            >
                            ...
                            <span
                              className={`font-medium ${timeLeft === "Ended" ? "text-ds-destructive" : "text-ds-warning"}`}
                            >
                              {timeLeft}
                            </span>
                          </div>
                        ...
                        <div className="flex justify-between items-center pt-3 mt-3 border-t border-ds-border">
                          ...
                          <span className="px-3 py-1.5 text-xs font-semibold text-white bg-ds-primary rounded-lg group-hover:bg-ds-primary/90 transition-colors ms-auto">
                            {t(locale, "auctions.place_bid")}
                          </span>
                        </div>
                      </div>
                    </a>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Find items you love from active and upcoming auctions across all
                categories.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Set your maximum bid and let the system bid on your behalf up to
                your limit.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                If you win, complete your payment securely and receive your
                item.
              </p>
            </div>
          </div>
