...
const fallbackItems = [
  ...
  },
  {
    ...
    topic: "shopping",
    thumbnail: "/seed-images/newsletters/1518770660439-4636190af475.jpg",
    edition_date: "2026-02-13",
    topics_covered: ["Best Deals", "Coupon Codes", "Product Reviews"],
    description:
      "Never miss a great deal — curated savings and product recommendations.",
  },
]
...
export const Route = createFileRoute("/$tenant/$locale/newsletter/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Subscribe to the Dakkah CityOS newsletter",
      },
    ],
  }),
  loader: async () => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm text-white/70 mb-4">
            ...
            <span className="text-white">
              {t(locale, "newsletter.breadcrumb")}
            </span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "newsletter.hero_title")}
          </h1>
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            <span>
              {items.length} {t(locale, "newsletter.editions_count")}
            </span>
            <span>|</span>
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "newsletter.search_label")}
                </label>
                <input
                  type="text"
                  value={searchQuery}
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "newsletter.topic_label")}
                </label>
            ...
            {filteredItems.length === 0 ? (
              <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
                ...
                <p className="text-ds-muted-foreground text-sm">
                  {t(locale, "newsletter.no_results_hint")}
                </p>
              </div>
            ) : (
                    ...
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-primary/10 to-ds-primary/10 relative overflow-hidden">
                      ...
                      {item.edition_date && (
                        <span className="absolute top-2 end-2 px-2 py-1 text-xs font-medium bg-ds-card/90 text-ds-foreground/80 rounded-md">
                          {new Date(item.edition_date!).toLocaleDateString(
                            "en-US",
                    ...
                    <div className="p-4">
                      ...
                      {item.topics_covered && (
                        <div className="mt-3">
                          <p className="text-xs text-ds-muted-foreground mb-1.5">
                            {t(locale, "newsletter.topics_covered")}:
                          </p>
                          <div className="flex flex-wrap gap-1.5">
                            {item.topics_covered.map((topic: string) => (
                              <span
                                key={topic}
                                className="px-2 py-0.5 bg-ds-info/10 text-ds-info rounded text-xs font-medium"
                              >
                                {topic}
                              </span>
                            ))}
                      ...
                      <div className="flex justify-between items-center pt-3 mt-3 border-t border-ds-border">
                        <span className="text-xs text-ds-muted-foreground">
                          {item.edition_date
                            ? new Date(item.edition_date!).toLocaleDateString()
                            : t(locale, "newsletter.recent_edition")}
                        </span>
                        <span className="px-4 py-1.5 text-xs font-semibold text-white bg-ds-primary rounded-lg group-hover:bg-ds-primary/90 transition-colors">
                          {t(locale, "newsletter.read_newsletter")}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      <section className="py-16 bg-ds-card border-t border-ds-border">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-ds-foreground text-center mb-12">
            {t(locale, "newsletter.stay_connected")}
          </h2>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "newsletter.step1_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "newsletter.step1_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-primary text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "newsletter.step2_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "newsletter.step2_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-primary text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "newsletter.step3_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "newsletter.step3_desc")}
              </p>
            </div>
          </div>
        </div>
