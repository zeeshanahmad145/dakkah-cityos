  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/newsletters/${params.id}`,
        {
...
function NewsletterDetailPage() {
  ...
  const sampleContent =
    newsletter.sample_content ||
    newsletter.preview ||
    newsletter.recent_issues ||
    []
            ...
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              {newsletter.thumbnail || newsletter.image || newsletter.cover ? (
                <img
                  ...
                  alt={newsletter.title || newsletter.name}
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {newsletter.topics.map((topic: string, idx: number) => (
                    <span
                      key={idx}
                      className="px-3 py-1.5 text-xs font-medium rounded-full bg-ds-muted text-ds-muted-foreground"
                    >
                      {topic}
                    </span>
                  ))}
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="space-y-4">
                  {newsletter.testimonials.map((t: any, idx: number) => (
                    ...
                    >
                      <p className="text-sm text-ds-muted-foreground italic">
                        "{t.text || t.quote}"
                      </p>
                      <p className="text-sm font-medium text-ds-foreground mt-2">
                        — {t.author || t.name}
                      </p>
                    </div>
                  ))}
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                <div className="text-center">
                  ...
                  {newsletter.price ? (
                    <p className="text-3xl font-bold text-ds-foreground">
                      ${Number(newsletter.price || 0).toLocaleString()}
                      <span className="text-base font-normal text-ds-muted-foreground">
                        /{newsletter.billing_period || "month"}
                      </span>
                    </p>
                  ) : (
                ...
                {subscribed ? (
                  <div className="text-center py-4">
                    <svg
                      className="w-12 h-12 text-ds-success mx-auto mb-2"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                ...
                <div className="space-y-2 text-sm">
                  {newsletter.frequency && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-ds-foreground font-medium capitalize">
                        {newsletter.frequency}
                      </span>
                    </div>
                  )}
              ...
              <div className="bg-ds-info/10 border border-ds-info/30 rounded-xl p-6">
                <h3 className="font-semibold text-ds-primary mb-2">
                  No Spam, Ever
                </h3>
