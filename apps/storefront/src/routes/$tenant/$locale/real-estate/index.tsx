...
export const Route = createFileRoute("/$tenant/$locale/real-estate/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse real estate listings on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/real-estate/listings`,
        {
          headers: {
...
function RealEstatePage() {
  ...
  const filteredItems = items.filter((item: any) => {
    ...
    const matchesListing =
      listingFilter === "all" || item.listing_type === listingFilter
    const matchesProperty =
      propertyFilter === "all" || item.property_type === propertyFilter
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm text-white/70 mb-4">
            ...
            <span className="text-white">
              {t(locale, "realEstate.breadcrumb")}
            </span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "realEstate.hero_title")}
          </h1>
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            <span>
              {items.length} {t(locale, "realEstate.properties_count")}
            </span>
            <span>|</span>
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "realEstate.search_label")}
                </label>
                <input
                  type="text"
                  value={searchQuery}
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "realEstate.listing_type_label")}
                </label>
                <div className="space-y-1">
                  {listingTypeOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "verticals.all_listings")
                        : opt === "sale"
                          ? t(locale, "verticals.for_sale")
                          : t(locale, "verticals.for_rent")}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "realEstate.property_type_label")}
                </label>
                <div className="space-y-1">
            ...
            {filteredItems.length === 0 ? (
              <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
                ...
                <p className="text-ds-muted-foreground text-sm">
                  {t(locale, "realEstate.no_results_hint")}
                </p>
              </div>
            ) : (
                ...
                {filteredItems.map((item: any) => (
                  ...
                  >
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-success/10 to-ds-success/15 relative overflow-hidden">
                      ...
                      {item.listing_type && (
                        <span
                          className={`absolute top-2 start-2 px-2 py-1 text-xs font-medium rounded-md ${item.listing_type === "sale" ? "bg-ds-success text-white" : "bg-ds-primary text-white"}`}
                        >
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-xl font-bold text-ds-success mt-2">
                        ...
                        {item.listing_type === "rent" && (
                          <span className="text-xs font-normal text-ds-muted-foreground">
                            {" "}
                            /month
                          </span>
                        )}
                      ...
                      <div className="flex justify-end items-center pt-3 mt-3 border-t border-ds-border">
                        <span className="px-3 py-1.5 text-xs font-semibold text-white bg-ds-success rounded-lg group-hover:bg-ds-success/90 transition-colors">
                          {t(locale, "verticals.view_details")}
                        </span>
                      </div>
                    </div>
                  </a>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "realEstate.step1_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "realEstate.step1_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-success text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "realEstate.step2_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "realEstate.step2_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-success text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "realEstate.step3_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "realEstate.step3_desc")}
              </p>
            </div>
          </div>
        </div>
