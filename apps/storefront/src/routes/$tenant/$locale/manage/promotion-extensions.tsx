...
function ManagePromotionExtPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/promotions-ext", {
        method: "GET",
      })
      return response
    },
  ...
  const allItems = ((data as any)?.promotions || (data as any)?.["promotion-ext"] || []).map(
    (item: any) => ({
      id: item.id,
    ...
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable
          columns={columns}
          data={items}
          emptyTitle="No promotion extensions found"
          countLabel="promotion extensions"
        />
      </Container>
      <FormDrawer
        open={drawerOpen}
