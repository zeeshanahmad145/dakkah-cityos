...
function ManageWalletPage() {
  ...
  const allItems = (data?.wallets || data?.wallet || []).map((item: any) => ({
    id: item.id,
    user:
      item.user || item.user_name || item.customer_name || item.email || "—",
    balance: item.balance ? `$${(item.balance / 100).toFixed(2)}` : "$0.00",
    pending: item.pending ? `$${(item.pending / 100).toFixed(2)}` : "$0.00",
    last_transaction:
      item.last_transaction || item.last_transaction_date
        ? new Date(
            item.last_transaction || item.last_transaction_date,
          ).toLocaleDateString()
        : "—",
