...
function ManageCartExtensionsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/cart-extensions", {
        method: "GET",
      })
      return response
    },
  ...
  const allItems = ((data as any)?.["cart-extensions"] || (data as any)?.carts || []).map(
    (item: any) => ({
      id: item.id,
    ...
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable
          columns={columns}
          data={items}
          emptyTitle="No cart extensions found"
          countLabel="cart extensions"
        />
      </Container>
      <FormDrawer
        open={drawerOpen}
