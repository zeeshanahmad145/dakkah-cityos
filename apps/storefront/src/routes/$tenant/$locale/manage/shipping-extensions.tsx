...
function ManageShippingExtPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/shipping-ext", {
        method: "GET",
      })
      return response
    },
  ...
  const allItems = ((data as any)?.["shipping-ext"] || (data as any)?.shipping || []).map(
    (item: any) => ({
      id: item.id,
    ...
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable
          columns={columns}
          data={items}
          emptyTitle="No shipping extensions found"
          countLabel="shipping extensions"
        />
      </Container>
      <FormDrawer
        open={drawerOpen}
