...
function ManageHealthcarePage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/healthcare", {
        method: "GET",
      })
      return response
    },
  ...
  const allItems = ((data as any)?.services || (data as any)?.healthcare || []).map(
    (item: any) => ({
      id: item.id,
    ...
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable
          columns={columns}
          data={items}
          emptyTitle="No healthcare services found"
          countLabel="services"
        />
      </Container>
      <FormDrawer
