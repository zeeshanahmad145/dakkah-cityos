...
function ManageTaxConfigPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/tax-config", {
        method: "GET",
      })
      return response
    },
  ...
  const allItems = ((data as any)?.["tax-config"] || (data as any)?.taxes || []).map(
    (item: any) => ({
      id: item.id,
    ...
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable
          columns={columns}
          data={items}
          emptyTitle="No tax configurations found"
          countLabel="tax configurations"
        />
      </Container>
      <FormDrawer
        open={drawerOpen}
