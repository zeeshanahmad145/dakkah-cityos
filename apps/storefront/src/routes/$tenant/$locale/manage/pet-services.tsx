...
function ManagePetServicesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.client.fetch("/admin/pet-services", {
        method: "GET",
      })
      return response
    },
  ...
  const allItems = ((data as any)?.services || (data as any)?.pet_services || []).map(
    (item: any) => ({
      id: item.id,
    ...
    <ManageLayout locale={locale}>
      <Container>
        ...
        <DataTable
          columns={columns}
          data={items}
          emptyTitle="No pet services found"
          countLabel="services"
        />
      </Container>
      <FormDrawer
