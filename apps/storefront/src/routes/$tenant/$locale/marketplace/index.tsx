...
export const Route = createFileRoute("/$tenant/$locale/marketplace/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse product categories on our marketplace",
      },
    ],
  }),
  loader: async () => {
    try {
      ...
      const items = raw.map((v: any) => {
        ...
        return {
          ...
          name:
            v.business_name || v.name || meta.business_name || "Unnamed Vendor",
      ...
      <div className="bg-gradient-to-r from-ds-primary to-ds-primary/90 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-lg text-white/80 max-w-2xl mx-auto">
            Explore our curated selection of products across multiple categories
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            ...
            <span>
              {items
                .reduce(
                  (sum: number, item: any) => sum + (item.total_products || 0),
                  0,
                )
                .toLocaleString()}{" "}
              products
            ...
            <span>|</span>
            <span>
              {t(locale, "marketplace.quality_assured", "Quality assured")}
            </span>
          </div>
        </div>
      </div>
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "marketplace.search_label", "Search")}
                </label>
                <input
                  type="text"
                  value={searchQuery}
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "marketplace.sort_label", "Sort by")}
                </label>
                <div className="space-y-1">
                  {sortOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "marketplace.all_categories", "All")
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "marketplace.price_label", "Price Range")}
                </label>
                <div className="space-y-1">
                  {priceOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "marketplace.all_prices", "All Prices")
                        : opt
                            .replace("_", " ")
                            .replace("under", "Under $")
                            .replace("to", "-$")
                    ...
                    <div className="p-4 relative">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0 pt-1">
                          <h3 className="text-lg font-semibold text-ds-foreground group-hover:text-ds-primary transition-colors line-clamp-1">
                            {item.name}
                          </h3>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-ds-muted-foreground mt-3 mb-4">
                        ...
                        <span>
                          {(item.total_products || 0).toLocaleString()} products
                        </span>
                      </div>

                      <div className="pt-3 border-t border-ds-border">
                        <span className="px-4 py-1.5 text-xs font-semibold text-white bg-ds-primary rounded-lg group-hover:bg-ds-primary/90 transition-colors inline-block">
                          Visit Store
                        </span>
                      </div>
                    </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Shop from thousands of products across all major categories.
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-primary text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <p className="text-sm text-ds-muted-foreground">
                All sellers are verified and quality-checked for your peace of
                mind.
              </p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-ds-muted-foreground">
                Protected transactions and hassle-free returns on all purchases.
              </p>
            </div>
          </div>
        </div>
