...
function GiftCardDetailPage() {
  ...
  const handleSendAsGift = async () => {
    ...
    try {
      await new Promise(r => setTimeout(r, 500))
      toast.success("Gift card ready to send!")
    } catch { toast.error("Something went wrong. Please try again.") }
    finally { setLoading(false) }
  }
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                ...
                <button onClick={handleSendAsGift} disabled={loading} className="w-full py-3 px-4 border border-ds-border text-ds-foreground rounded-lg font-medium hover:bg-ds-muted transition-colors flex items-center justify-center gap-2 disabled:opacity-50">
