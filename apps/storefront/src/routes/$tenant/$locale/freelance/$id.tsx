  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/freelance/${params.id}`,
        {
...
function FreelanceDetailPage() {
  ...
  const handleOrder = async (price?: number) => {
    ...
    try {
      toast.success(
        `Order placed for $${Number(price || gig?.price || 0).toLocaleString()}! The seller will be in touch shortly.`,
      )
    } finally {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This freelance listing may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/freelance` as never}
              ...
              {tiers.length > 0 ? (
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    {tiers[selectedTier]?.delivery_time && (
                      <div className="flex items-center gap-1.5 text-sm text-ds-muted-foreground">
                        ...
                        <span>
                          {tiers[selectedTier].delivery_time} day delivery
                        </span>
                      </div>
                    )}
                    {tiers[selectedTier]?.features && (
                      <ul className="space-y-2">
                        {tiers[selectedTier].features.map(
                ...
                <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                  <p className="text-3xl font-bold text-ds-foreground text-center">
                    {gig.price != null
                      ? `$${Number(gig.price || 0).toLocaleString()}`
                      : "Contact for price"}
                  </p>
                  {gig.delivery_time && (
