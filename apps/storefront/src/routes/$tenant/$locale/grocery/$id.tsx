  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/grocery/${params.id}`,
        {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This grocery product may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/grocery` as never}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="relative aspect-square sm:aspect-[4/3] bg-ds-muted rounded-xl overflow-hidden">
              ...
              {product.category && (
                <span className="absolute top-4 end-4 px-3 py-1 text-xs font-medium rounded-full bg-ds-background/80 text-ds-foreground backdrop-blur-sm">
                  {product.category}
                </span>
              )}
            </div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(product.nutrition).map(
                      ([key, value]: [string, any]) => (
                        <div
                          key={key}
                          className="flex items-center justify-between p-2 text-sm border-b border-ds-border last:border-0"
                        >
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.allergens.map((allergen: string, idx: number) => (
                    <span
                      key={idx}
                      className="px-3 py-1 text-xs font-medium rounded-full bg-ds-warning/20 text-ds-warning"
                    >
                      {allergen}
                    </span>
                  ))}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {product.related_products.map(
                      (related: any, idx: number) => (
                        <div
                          key={idx}
                          className="p-3 border border-ds-border rounded-lg"
                        >
                          ...
                          {related.price != null && (
                            <p className="text-sm text-ds-primary font-bold mt-1">
            ...
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                ...
                {product.in_stock != null && (
                  <div className="text-center">
                    <span
                      className={`inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold rounded-full ${product.in_stock ? "bg-ds-success/20 text-ds-success" : "bg-ds-destructive/20 text-ds-destructive"}`}
                    >
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <span className="text-lg font-semibold text-ds-foreground w-12 text-center">
                    {quantity}
                  </span>
                  <button
                ...
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.dietary.map((diet: string, idx: number) => (
                      <span
                        key={idx}
                        className="px-3 py-1 text-xs font-medium rounded-full bg-ds-success/10 text-ds-success"
                      >
                        {diet}
                      </span>
                    ))}
