...
function GroceryPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    ...
    const matchesOrganic =
      organicFilter === "all" ||
      (organicFilter === "yes" ? item.organic : !item.organic)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm text-white/70 mb-4">
            ...
            <span className="text-white">
              {t(locale, "grocery.breadcrumb")}
            </span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "grocery.hero_title")}
          </h1>
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            <span>
              {items.length} {t(locale, "grocery.products_available")}
            </span>
            <span>|</span>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-72 flex-shrink-0">
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "grocery.category_label")}
                </label>
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "grocery.organic_label")}
                </label>
                <div className="space-y-1">
                  {organicOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "verticals.all_products")
                        : opt === "yes"
                          ? t(locale, "verticals.organic_only")
                          : t(locale, "verticals.non_organic")}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {filteredItems.length === 0 ? (
              <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
                ...
                <p className="text-ds-muted-foreground text-sm">
                  {t(locale, "grocery.no_results_hint")}
                </p>
              </div>
            ) : (
                    ...
                    <div className="aspect-[4/3] bg-gradient-to-br from-ds-success/10 to-lime-100 relative overflow-hidden">
                      ...
                      {item.organic && (
                        <span className="absolute top-2 start-2 px-2 py-1 text-xs font-medium bg-ds-success text-white rounded-md">
                          🌿 {t(locale, "grocery.organic_badge")}
                        </span>
                      )}
                      {item.category && (
      ...
      <section className="py-16 bg-ds-card border-t border-ds-border">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-ds-foreground text-center mb-12">
            {t(locale, "grocery.why_shop_title")}
          </h2>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "grocery.step1_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "grocery.step1_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-success text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "grocery.step2_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "grocery.step2_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-success text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "grocery.step3_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "grocery.step3_desc")}
              </p>
            </div>
          </div>
        </div>
