  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/vendors/${params.id}`,
        {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This vendor profile may have been removed or is no longer
              available.
            </p>
            <Link
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-ds-background border border-ds-border rounded-xl p-6">
              ...
              {vendor.description && (
                <p className="mt-4 text-ds-muted-foreground text-sm leading-relaxed whitespace-pre-wrap">
                  {vendor.description}
                </p>
              )}
            </div>
              ...
              {productsLoading ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div
                      key={i}
                      className="aspect-square bg-ds-muted rounded-lg animate-pulse"
                    />
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {products.map((product: any) => (
                    <Link
                      key={product.id}
                      to={`${prefix}/products/${product.handle || product.id}` as never}
                    ...
                    >
                      <div className="aspect-square bg-ds-muted overflow-hidden">
                        {product.thumbnail ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center">
                            <svg
                              className="w-10 h-10 text-ds-muted-foreground/30"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                            >
                      ...
                      <div className="p-3">
                        <h3 className="text-sm font-medium text-ds-foreground truncate">
                          {product.title}
                        </h3>
                        {product.price != null && (
                          <p className="text-sm font-semibold text-ds-primary mt-1">
              ...
              {reviews && reviews.length > 0 ? (
                <div className="space-y-4">
                  {reviews.map((review: any) => (
                    <div
                      key={review.id}
                      className="bg-ds-background border border-ds-border rounded-lg p-4"
                    >
                      <div className="flex items-center gap-3 mb-2">
                        <div className="w-8 h-8 bg-ds-muted rounded-full flex items-center justify-center text-xs font-semibold text-ds-foreground">
                          {(review.customer_name || review.author || "A")
                            .charAt(0)
                            .toUpperCase()}
                        </div>
                        <div>
                      ...
                      {review.title && (
                        <p className="text-sm font-medium text-ds-foreground mb-1">
                          {review.title}
                        </p>
                      )}
                      {review.content && (
