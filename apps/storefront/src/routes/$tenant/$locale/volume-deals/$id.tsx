  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/volume-deals/${params.id}`,
        {
...
function VolumeDealsDetailPage() {
  ...
  const handleRequestQuote = async () => {
    setLoading(true)
    try {
      toast.success("Custom quote request sent!")
    } catch {
      toast.error("Something went wrong. Please try again.")
    } finally {
      setLoading(false)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This volume deal may have been removed or is no longer available.
            </p>
            <Link
              to={`${prefix}/volume-deals` as never}
  ...
  const savings =
    basePrice && currentPrice
      ? ((1 - Number(currentPrice) / Number(basePrice)) * 100).toFixed(0)
      : 0
            ...
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              {item.thumbnail || item.image ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-ds-warning/10 to-ds-warning/15">
                  <svg
                    className="w-16 h-16 text-ds-warning"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-ds-border">
                        <th className="text-left py-3 text-ds-muted-foreground font-medium">
                          Quantity
                        </th>
                        <th className="text-left py-3 text-ds-muted-foreground font-medium">
                          Price per Unit
                      ...
                      {tiers.map((tier: any, idx: number) => {
                        const tierSavings =
                          basePrice && tier.price
                            ? (
                                (1 - Number(tier.price) / Number(basePrice)) *
                                100
                              ).toFixed(0)
                            : 0
                        ...
                        return (
                          <tr
                            key={idx}
                            className={`border-b border-ds-border last:border-0 ${isActive ? "bg-ds-primary/5" : ""}`}
                          >
                            <td className="py-3 text-ds-foreground">
                              {tier.min_quantity || tier.min || 0}
                              {tier.max_quantity || tier.max
                                ? `-${tier.max_quantity || tier.max}`
                                : "+"}{" "}
                              units
                              {isActive && (
                                <span className="ms-2 text-xs text-ds-primary font-medium">
                                  Current
                                </span>
                              )}
                            </td>
                            <td className="py-3 text-ds-foreground font-medium">
                              ${Number(tier.price || 0).toLocaleString()}
                            </td>
                            <td className="py-3 text-ds-success font-medium">
                              {tierSavings > 0 ? `${tierSavings}% off` : "-"}
                            </td>
                          </tr>
                        )
                      })}
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                <div>
                  <label className="text-sm text-ds-muted-foreground block mb-2">
                    Quantity
                  </label>
                  <input
                    type="number"
                ...
                <div className="border-t border-ds-border pt-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-lg font-bold text-ds-foreground">
                      ...
                      {currentPrice != null
                        ? (
                            Number(currentPrice || 0) * quantity
                          ).toLocaleString()
                        : "—"}
                    </span>
                  </div>
