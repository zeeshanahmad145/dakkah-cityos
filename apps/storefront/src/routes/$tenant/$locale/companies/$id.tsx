  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/companies/${params.id}`,
        {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This company profile may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/b2b` as never}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="relative aspect-[16/9] bg-ds-muted rounded-xl overflow-hidden">
              ...
              {item.tier && (
                <span
                  className={`absolute top-4 start-4 px-3 py-1 text-xs font-semibold rounded-full capitalize ${tierColors[item.tier] || "bg-gray-100 text-gray-800"}`}
                >
                  {item.tier} Tier
                </span>
              )}
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                {item.status && (
                  <span
                    className={`px-2 py-0.5 text-xs font-medium rounded-full capitalize ${statusColors[item.status] || "bg-gray-100 text-gray-800"}`}
                  >
                    {item.status}
                  </span>
                )}
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-ds-foreground font-medium">
                      ${(item.credit_limit / 100).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-ds-muted-foreground">
                    ...
                    <span className="text-ds-foreground font-medium">
                      ${((item.credit_used || 0) / 100).toLocaleString()}
                    </span>
                  </div>
                  <div className="w-full bg-ds-muted rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, ((item.credit_used || 0) / item.credit_limit) * 100)}%`,
                      }}
                    />
                  </div>
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-green-600 font-medium">
                      $
                      {(
                        (item.credit_limit - (item.credit_used || 0)) /
                        100
                      ).toLocaleString()}
                    </span>
                  </div>
            ...
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                ...
                >
                  ...
                  {loading ? "Sending..." : "Contact Company"}
                </button>
                ...
                <div className="space-y-3 text-sm">
                  {item.email && (
                    <div className="flex items-center gap-2">
                      <svg
                        className="w-4 h-4 text-ds-muted-foreground"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
