...
function RestaurantDetailPage() {
  ...
  const handleReserveTable = () => {
    toast.success("Table reservation request submitted!")
  }

  const handleOrderOnline = async () => {
    setLoading(true)
    try {
      const resp = await fetch(`${baseUrl}/store/restaurants/${id}/order`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-publishable-api-key": publishableKey },
        credentials: "include",
        body: JSON.stringify({ restaurant_id: id })
      })
      if (resp.ok) toast.success("Order placed successfully!")
  ...
  const handleCallRestaurant = () => {
    if (restaurant?.phone) {
      ...
      toast.info("Phone number not available. Please try ordering online.")
    }
  }
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                <button onClick={handleReserveTable} className="w-full py-3 px-4 bg-ds-primary text-ds-primary-foreground rounded-lg font-medium hover:bg-ds-primary/90 transition-colors flex items-center justify-center gap-2">
                ...
                <button onClick={handleOrderOnline} disabled={loading} className="w-full py-3 px-4 border border-ds-border text-ds-foreground rounded-lg font-medium hover:bg-ds-muted transition-colors flex items-center justify-center gap-2 disabled:opacity-50">
                  ...
                  {loading ? "Ordering..." : "Order Online"}
                </button>

                <button onClick={handleCallRestaurant} className="w-full py-3 px-4 border border-ds-border text-ds-foreground rounded-lg font-medium hover:bg-ds-muted transition-colors flex items-center justify-center gap-2">
