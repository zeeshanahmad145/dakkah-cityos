  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/restaurants/${params.id}`,
        {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This restaurant listing may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/restaurants` as never}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-3">
                {restaurant.cuisine && (
                  <span className="px-3 py-1 text-xs font-medium rounded-full bg-ds-primary/10 text-ds-primary">
                    {restaurant.cuisine}
                  </span>
                )}
                {priceDisplay && (
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="space-y-6">
                  {restaurant.menu.map((section: any, idx: number) => (
                    <div key={idx}>
                      <h3 className="text-sm font-semibold text-ds-foreground mb-3 uppercase tracking-wide">
                        {section.category || section.name}
                      </h3>
                      <div className="space-y-3">
                        {(section.items || []).map(
                          (menuItem: any, mIdx: number) => (
                            <div
                              key={mIdx}
                              className="flex items-start justify-between gap-4 py-2 border-b border-ds-border last:border-0"
                            >
                              <div>
              ...
              {restaurant.features && (restaurant.features as string[]).length > 0 && (
                <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(restaurant.features as string[]).map((feature: string, idx: number) => (
                      <span
                        key={idx}
