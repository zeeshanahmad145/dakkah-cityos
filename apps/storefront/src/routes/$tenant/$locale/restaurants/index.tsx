...
export const Route = createFileRoute("/$tenant/$locale/restaurants/")({
  ...
  loader: async () => {
    try {
      ...
      const raw =
        data.items || data.restaurants || data.listings || data.products || []
      const items = raw.map((s: any) => {
        ...
        return {
          ...
          images: meta.images || [],
          cuisine_type:
            s.cuisine_type || s.cuisine_types || meta.cuisine_type || null,
          ...
          delivery_available:
            s.delivery_available || meta.delivery_available || false,
          pickup_available:
            s.pickup_available || meta.pickup_available || false,
          dine_in_available:
            s.dine_in_available || meta.dine_in_available || false,
        }
      })
...
function RestaurantsPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    ...
    const matchesCuisine =
      cuisineFilter === "all" || item.cuisine_type === cuisineFilter
    const matchesPrice =
      priceFilter === "all" || item.price_range === priceFilter
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2 text-sm text-white/70 mb-4">
            ...
            <span className="text-white">
              {t(locale, "restaurant.breadcrumb")}
            </span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            {t(locale, "restaurant.hero_title")}
          </h1>
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            <span>
              {items.length} {t(locale, "restaurant.restaurants_count")}
            </span>
            <span>|</span>
            ...
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "restaurant.search_label")}
                </label>
                <input
                  type="text"
                  value={searchQuery}
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "restaurant.cuisine_label")}
                </label>
                <div className="space-y-1">
                  {cuisineOptions.map((opt) => (
                    ...
                    >
                      {opt === "all"
                        ? t(locale, "verticals.all_cuisines")
                        : opt
                            .split("_")
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "restaurant.price_label")}
                </label>
                <div className="space-y-1">
            ...
            {filteredItems.length === 0 ? (
              <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
                ...
                <p className="text-ds-muted-foreground text-sm">
                  {t(locale, "restaurant.no_results_hint")}
                </p>
              </div>
            ) : (
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center gap-2 mt-2 text-xs text-ds-muted-foreground">
                        {item.delivery_available && (
                          <span className="px-2 py-0.5 bg-ds-success/15 text-ds-success rounded-md font-medium">
                            Delivery
                          </span>
                        )}
                        {item.pickup_available && (
                          <span className="px-2 py-0.5 bg-ds-info/15 text-ds-info rounded-md font-medium">
                            Pickup
                          </span>
                        )}
                        {item.dine_in_available && (
                          <span className="px-2 py-0.5 bg-ds-primary/15 text-ds-primary rounded-md font-medium">
                            Dine-in
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex justify-between items-center pt-3 mt-3 border-t border-ds-border">
                        ...
                        <span className="px-3 py-1.5 text-xs font-semibold text-white bg-ds-warning rounded-lg group-hover:bg-ds-warning transition-colors ms-auto">
                          {t(locale, "restaurant.view_menu")}
                        </span>
                      </div>
                    </div>
                  </a>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "restaurant.step1_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "restaurant.step1_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-warning text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "restaurant.step2_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "restaurant.step2_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-warning text-white flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "restaurant.step3_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "restaurant.step3_desc")}
              </p>
            </div>
          </div>
        </div>
