...
function LegalDetailPage() {
  ...
  const handleScheduleConsultation = async () => {
    setLoading(true)
    try {
      const resp = await fetch(`${baseUrl}/store/legal/consultations`, {
        method: "POST",
        headers: {
      ...
      if (resp.ok) toast.success("Consultation scheduled successfully!")
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <p className="text-ds-muted-foreground mb-6">
              This legal service may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/legal` as never}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-3">
                  {service.attorney.name && (
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-ds-muted-foreground w-32">
                        Name:
                      </span>
                  ...
                  {service.attorney.education && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-ds-foreground">
                        {Array.isArray(service.attorney.education)
                          ? service.attorney.education.join(", ")
                          : service.attorney.education}
                      </span>
                    </div>
                  )}
            ...
            {service.education && !service.attorney && (
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="space-y-2">
                  {(Array.isArray(service.education)
                    ? service.education
                    : [service.education]
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {service.case_types.map((caseType: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 text-sm text-ds-muted-foreground p-2 bg-ds-muted/30 rounded-lg"
                    >
                      <svg
                        className="w-4 h-4 text-ds-primary flex-shrink-0"
                        fill="none"
          ...
          <aside className="space-y-6">
            <div className="sticky top-4 space-y-6">
              <div className="bg-ds-background border border-ds-border rounded-xl p-6 space-y-4">
                {service.hourly_rate != null && (
                  <div className="text-center">
                    <p className="text-3xl font-bold text-ds-foreground">
                      ${Number(service.hourly_rate || 0).toLocaleString()}
                    </p>
                ...
                {service.consultation_fee != null && (
                  <div className="text-center text-sm text-ds-muted-foreground">
                    ...
                    {Number(service.consultation_fee || 0) === 0
                      ? "Free"
                      : `$${Number(service.consultation_fee || 0).toLocaleString()}`}
                  </div>
                )}
              ...
              {service.bar_associations &&
                service.bar_associations.length > 0 && (
                  <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                    <h3 className="font-semibold text-ds-foreground mb-3">
                      Bar Associations
                    </h3>
                    <div className="space-y-2">
                      {service.bar_associations.map(
