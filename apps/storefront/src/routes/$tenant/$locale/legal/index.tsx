...
export const Route = createFileRoute("/$tenant/$locale/legal/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse legal services on Dakkah CityOS",
      },
    ],
  }),
  loader: async () => {
...
function LegalServicesPage() {
  ...
  const filteredItems = items.filter((item: any) => {
    const matchesSearch = searchQuery
      ...
      : true
    const specs = Array.isArray(item.specializations)
      ? item.specializations
      : item.specializations
        ? [item.specializations]
        : []
    const matchesSpec =
      specFilter === "all" || specs.some((s: string) => s === specFilter)
      ...
      <div className="bg-gradient-to-r from-ds-primary to-ds-primary/80 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-sm text-white/60">
            <span>
              {items.length} {t(locale, "legal.attorneys_count")}
            </span>
            <span>|</span>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-72 flex-shrink-0">
            <div className="bg-ds-background border border-ds-border rounded-xl p-4 space-y-6 sticky top-4">
              ...
              <div>
                <label className="block text-sm font-medium text-ds-foreground mb-2">
                  {t(locale, "legal.specialization_label")}
                </label>
            ...
            {filteredItems.length === 0 ? (
              <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
                ...
                <p className="text-ds-muted-foreground text-sm">
                  {t(locale, "legal.no_results_hint")}
                </p>
              </div>
            ) : (
                    ...
                    >
                      ...
                      {specs.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-3">
                          {specs.slice(0, 3).map((s: string) => (
                            <span
                              key={s}
                              className="px-2 py-0.5 text-xs font-medium bg-ds-muted text-ds-foreground rounded-md"
                            >
                              {formatSpec(s)}
                            </span>
                          ))}
                          {specs.length > 3 && (
                            <span className="px-2 py-0.5 text-xs font-medium bg-ds-muted text-ds-muted-foreground rounded-md">
                              +{specs.length - 3}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex justify-between items-center pt-3 mt-3 border-t border-ds-border">
                        ...
                        <span className="px-3 py-1.5 text-xs font-semibold text-ds-primary-foreground bg-ds-primary rounded-lg group-hover:bg-ds-primary/80 transition-colors">
                          {t(locale, "legal.book_consultation")}
                        </span>
                      </div>
                    </a>
                  )
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "legal.step1_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "legal.step1_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-primary text-ds-primary-foreground flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "legal.step2_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "legal.step2_desc")}
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-ds-primary text-ds-primary-foreground flex items-center justify-center text-xl font-bold mx-auto mb-4">
              ...
              <h3 className="font-semibold text-ds-foreground mb-2">
                {t(locale, "legal.step3_title")}
              </h3>
              <p className="text-sm text-ds-muted-foreground">
                {t(locale, "legal.step3_desc")}
              </p>
            </div>
          </div>
        </div>
