  ...
  loader: async ({ params }) => {
    try {
      ...
      const resp = await fetchWithTimeout(
        `${baseUrl}/store/trade-in/${params.id}`,
        {
...
function TradeInDetailPage() {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-ds-background border border-ds-border rounded-xl p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-ds-foreground mb-2">
              Trade-In Item Not Found
            </h2>
            <p className="text-ds-muted-foreground mb-6">
              This trade-in item may have been removed or is no longer
              available.
            </p>
            <Link
              to={`${prefix}/trade-in` as never}
  ...
  const conditionClass =
    conditionColors[(item.condition || "").toLowerCase()] ||
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-3">
                {item.condition && (
                  <span
                    className={`px-3 py-1 text-xs font-medium rounded-full ${conditionClass}`}
                  >
              ...
              <div className="bg-ds-background border border-ds-border rounded-xl p-6">
                ...
                <ul className="space-y-3 text-sm text-ds-muted-foreground">
                  <li className="flex items-start gap-3">
                    <span className="w-6 h-6 bg-ds-primary/10 text-ds-primary rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0">
                      1
                    </span>
