...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (!customerId) {
    return res.json({
      ...
      public_info: {
        ...
        description:
          "Manage your store credits, gift cards, and loyalty points all in one place.",
        features: [
  ...
  try {
    ...
    const promotionExt = req.scope.resolve("promotionExt") as unknown as any;
    ...
    try {
      ...
      const gcList = Array.isArray(giftCards)
        ? giftCards
        : [giftCards].filter(Boolean);
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    const giftCard = await promotionExt.createGiftCardExts({
      tenant_id,
      code,
