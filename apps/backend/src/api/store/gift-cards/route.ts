...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    return res.json({
      items: SEED_GIFT_CARDS,
      gift_cards: SEED_GIFT_CARDS,
      count: SEED_GIFT_CARDS.length,
      limit: 20,
      offset: 0,
    });
  }
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    if (code) {
      const cards = await moduleService.listGiftCardExts({
        code,
        is_active: true,
      });
      ...
      if (cardList.length === 0) {
        return res
          .status(404)
          .json({ message: "Gift card not found or inactive" });
      }
      ...
      if (Number(giftCard.remaining_value) <= 0) {
        return res
          .status(400)
          .json({ message: "Gift card has no remaining balance" });
      }
      ...
      if (redeemAmount > Number(giftCard.remaining_value)) {
        return res
          .status(400)
          .json({ message: "Insufficient gift card balance" });
      }

      const newBalance = Number(giftCard.remaining_value) - redeemAmount;
      await moduleService.updateGiftCardExts({
        id: giftCard.id,
    ...
    if (recipient_email && amount && tenant_id) {
      ...
      const giftCard = await moduleService.createGiftCardExts({
        tenant_id,
        code: generatedCode,
    ...
    return res.status(400).json({
      message:
        "Provide a code to redeem, or recipient_email, amount, and tenant_id to purchase",
    });
  } catch (error: unknown) {
