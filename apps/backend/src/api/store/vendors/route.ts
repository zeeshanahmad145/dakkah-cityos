...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    if (effectiveMarketplaceId) {
      try {
        const marketplace = await tenantModule.retrieveTenant(
          effectiveMarketplaceId,
        );
        ...
        const canHostVendors =
          marketplace.can_host_vendors === true ||
    ...
    const result =
      publicVendors.length > 0
        ? sanitizeList(publicVendors, "vendors")
        : SEED_VENDORS;
    res.json({
      vendors: result,
      items: result,
