...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  try {
    const walletModule = req.scope.resolve("wallet") as any
    ...
    const { customer_id, status } = req.query as Record<string, string | undefined>
    ...
    const wallets = await walletModule.listWallets(filters, { skip: offset, take: limit })
