...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    const { type, is_active, vendor_id } = req.query as Record<string, string | undefined>
    if (type) filters.commission_type = type
    if (is_active !== undefined) filters.status = is_active === "true" ? "active" : "inactive"
    if (vendor_id) filters.vendor_id = vendor_id

    const limit = parseInt(req.query.limit as string) || 20
    const offset = parseInt(req.query.offset as string) || 0

    const rules = await commissionModule.listCommissionRules(filters, { skip: offset, take: limit })
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    const rule = await commissionModule.createCommissionRules({
      ...
      commission_percentage: parsed.data.type === "percentage" ? parsed.data.rate : undefined,
      commission_flat_amount: parsed.data.type === "flat" ? parsed.data.rate : undefined,
