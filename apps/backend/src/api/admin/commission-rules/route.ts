...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    const raw = await commissionService.createCommissionRules(parseResult.data);
    const commission_rule = Array.isArray(raw) ? raw[0] : raw;
    logger.info("Commission rule created", { ruleId: commission_rule.id });
    res.status(201).json({ commission_rule });
  } catch (error: unknown) {
