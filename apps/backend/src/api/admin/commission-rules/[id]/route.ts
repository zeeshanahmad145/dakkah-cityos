...
export async function PUT(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    if (min_order_value !== undefined)
      updateData.min_order_value = min_order_value;
    if (max_order_value !== undefined)
      updateData.max_order_value = max_order_value;
    if (is_active !== undefined) updateData.is_active = is_active;
    if (priority !== undefined) updateData.priority = priority;

    const rule = await commissionService.updateCommissions(updateData);
...
export async function DELETE(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    await commissionService.deleteCommissions(id);
