...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    const { id } = req.params;
    const [item] = await mod.listGiftCardExts({ id }, { take: 1 });
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    const mod = req.scope.resolve("promotionExt") as unknown as any;
    const { id } = req.params;
    const raw = await mod.updateGiftCardExts({ id, ...(req.body as Record<string, unknown>) });
...
export async function DELETE(req: MedusaRequest, res: MedusaResponse) {
  try {
    const mod = req.scope.resolve("promotionExt") as unknown as any;
    const { id } = req.params;
    try { await mod.deleteGiftCardExts([id]); } catch { /* soft-delete fallback */ }
