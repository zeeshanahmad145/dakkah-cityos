...
function serveSeedFallback(res: MedusaResponse, blobPath: string): boolean {
  ...
  const fallbackKey =
    Object.keys(CATEGORY_FALLBACK_MAP).find(
      (k) =>
        filename.toLowerCase().includes(k) ||
        pathSegments.some((s) => s.toLowerCase().includes(k)),
    ) || "default";
...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    const contentLength =
      result.blob?.size || result.headers?.get("content-length");
    ...
    if (
      ...
      (err as NodeJS.ErrnoException)?.name === "BlobNotFoundError"
    ) {
      if (serveSeedFallback(res, blobPath)) return;
      return res.status(404).json({ error: "File not found" });
    }
