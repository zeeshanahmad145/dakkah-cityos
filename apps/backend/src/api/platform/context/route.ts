...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  const tenantSlug = (req.query?.tenant ||
    req.query?.tenantId ||
    req.headers["x-cityos-tenant-id"] ||
    DEFAULT_TENANT_SLUG) as string;
  const nodeId = (req.query?.node ||
    req.query?.nodeId ||
    req.headers["x-cityos-node-id"]) as string | undefined;
  ...
  try {
    ...
    try {
      nodeModule = req.scope.resolve("node") as unknown as any;
    } catch {}
    try {
      governanceModule = req.scope.resolve("governance") as unknown as any;
    } catch {}
    ...
    try {
      ...
      console.log(
        "[PLATFORM-CONTEXT] listTenants({}) raw type:",
        typeof rawResult,
        "isArray:",
      ...
      if (Array.isArray(rawResult)) {
        console.log(
          "[PLATFORM-CONTEXT] listTenants count:",
          rawResult.length,
          "first:",
          rawResult[0]?.slug,
          rawResult[0]?.status,
        ...
        if (rawResult.length > 0 && Array.isArray(rawResult[0])) {
          console.log(
            "[PLATFORM-CONTEXT] listTenants returns tuple! inner:",
            rawResult[0].length,
          );
        }
      }
      ...
      console.log(
        "[PLATFORM-CONTEXT] listTenants({slug:'" + tenantSlug + "'}) raw type:",
        typeof rawBySlug,
        "isArray:",
        Array.isArray(rawBySlug),
      );
      if (Array.isArray(rawBySlug)) {
        console.log(
          "[PLATFORM-CONTEXT] bySlug count:",
          rawBySlug.length,
          "first:",
          rawBySlug[0]?.slug,
        );
        ...
        console.log(
          "[PLATFORM-CONTEXT] bySlug value:",
          JSON.stringify(rawBySlug)?.substring(0, 200),
        );
      }
    } catch (debugErr: any) {
    ...
    try {
      ...
      console.error(
        "[PLATFORM-CONTEXT] resolveTenant error:",
    ...
    if (!tenant) {
      try {
        ...
        console.error(
          "[PLATFORM-CONTEXT] listTenants fallback error:",
    ...
    if (!tenant) {
      return res
        .status(503)
        .json({ success: false, message: "Platform tenant unavailable" });
    }
    ...
    if (nodeModule) {
      try {
        ...
        const flatNodes = await nodeModule.listNodesByTenant(
          tenant.id,
          filters,
        );
    ...
    if (governanceModule) {
      try {
        const effectivePolicies =
          await governanceModule.resolveEffectivePolicies(tenant.id);
        let authorities: any[] = [];
        try {
          const rawAuthorities =
            await governanceModule.listGovernanceAuthorities({
              tenant_id: tenant.id,
            });
