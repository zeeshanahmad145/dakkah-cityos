...
async function handleInventoryUpdate(payload: any, container: any) {
  const log = container.resolve("logger") as unknown as any;
  try {
        ...
        try {
          ...
          if (inventoryItems?.length > 0) {
            ...
            if (locationLevel) {
              ...
              log.info(
                `Updated inventory for ${item_code}: stocked_quantity=${availableQty}, warehouse=${warehouse || "default"}`,
              );
            } else {
              log.warn(
                `No location level found for inventory item ${item_code}, skipping stock update`,
              );
            }
          } else {
            log.warn(
              `No inventory item found for SKU ${item_code}, skipping stock update`,
            );
          }
        } catch (invError) {
    ...
    log.error("Failed to process ERPNext inventory update", {
...
async function handleInvoiceStatusChange(payload: any, container: any) {
  ...
  try {
    ...
    if (new_status === "Paid") {
      ...
      log.warn(
        `Invoice ${invoice_id} cancelled for order ${order.id} — flagged for review`,
      );
    }
    ...
    log.info(
      `ERPNext invoice status change processed: ${invoice_id} → ${new_status}`,
    );
  } catch (error) {
    log.error("Failed to process ERPNext invoice status change", {
      error: error instanceof Error ? (error instanceof Error ? error.message : String(error)) : error,
    });
  }
}
...
async function handlePaymentEntry(payload: any, container: any) {
  ...
  try {
    ...
    const medusa_order_id =
      payload.custom_medusa_order_id || payload.medusa_order_id;

    if (!reference_no && !medusa_order_id) {
      log.warn(
        "ERPNext payment entry missing reference_no and medusa_order_id",
      );
      return;
    }
    ...
    if (!order) {
      log.info(
        `No matching order found for ERPNext payment entry ${reference_no || medusa_order_id}`,
      );
      return;
    }
    ...
    if (fullyPaid) {
      ...
      log.info(
        `Order ${order.id} fully paid (${totalPaid}/${orderTotal}) — ready for fulfillment`,
      );
    }
    ...
    log.info(
      `ERPNext payment entry processed: ${reference_no}, amount=${amount}, mode=${mode_of_payment}`,
    );
  } catch (error) {
    log.error("Failed to process ERPNext payment entry", {
      error: error instanceof Error ? (error instanceof Error ? error.message : String(error)) : error,
    });
  }
}
...
async function handleStockReconciliation(payload: any, container: any) {
  ...
  try {
    ...
    if (!items.length) {
      log.warn(
        `ERPNext stock reconciliation ${reconciliationName} has no items`,
      );
      return;
    }
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (!products?.length) {
          log.info(
            `Stock reconciliation: no Medusa product found for item_code=${item_code}`,
          );
          continue;
        }
        ...
        if (diff > 0 && diff <= DISCREPANCY_THRESHOLD) {
          ...
          log.info(
            `Auto-adjusting inventory for ${item_code}: diff=${diff} (within threshold ±${DISCREPANCY_THRESHOLD})`,
          );
          ...
          log.warn(
            `Inventory discrepancy for ${item_code}: erpnext=${erpnext_qty}, current=${item.current_qty || 0}, diff=${diff} — requires manual review`,
          );
        }
      } catch (itemError) {
        log.error(`Error processing reconciliation item ${item_code}`, {
    ...
    log.info(
      `ERPNext stock reconciliation ${reconciliationName} processed: ${items.length} items, ${autoAdjusted.length} auto-adjusted, ${discrepancies.length} discrepancies flagged`,
    );
  } catch (error) {
    log.error("Failed to process ERPNext stock reconciliation", {
      error: error instanceof Error ? (error instanceof Error ? error.message : String(error)) : error,
    });
  }
}
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    if (secret) {
      const signature =
        (req.headers["x-erpnext-signature"] as string) ||
        (req.headers["x-erpnext-secret"] as string);
      if (!signature) {
        logger.info(
          `[Webhook:ERPNext] Missing signature header (correlation: ${correlationId})`,
        );
      ...
      try {
        if (!verifyHMAC(rawBody, signature, secret)) {
          logger.info(
            `[Webhook:ERPNext] HMAC verification failed (correlation: ${correlationId})`,
          );
          return res.status(400).json({ error: "Invalid signature" });
        }
      } catch {
        logger.info(
          `[Webhook:ERPNext] Signature verification error (correlation: ${correlationId})`,
        );
        return res.status(400).json({ error: "Invalid signature" });
      }
    }
    ...
    logger.info(
      `[Webhook:ERPNext] Received event: ${event} (correlation: ${correlationId})`,
    );
    ...
    switch (event) {
      ...
      default:
        logger.info(
          `[Webhook:ERPNext] Unhandled event: ${event} (correlation: ${correlationId})`,
        );
        break;
    }
    ...
    try {
      ...
      logger.info(
        `[Webhook:ERPNext] Temporal dispatch skipped: ${(error instanceof Error ? error.message : String(error))}`,
      );
    }

    return res
      .status(200)
      .json({ received: true, event, correlation_id: correlationId });
  } catch (error: unknown) {
