...
async function handleDeliveryStatusUpdate(payload: any, container: any) {
  ...
  try {
    ...
    const updatedMetadata = {
      ...
      fleetbase_tracking_number:
        tracking_number || order.metadata?.fleetbase_tracking_number,
      fleetbase_estimated_delivery:
        estimated_delivery || order.metadata?.fleetbase_estimated_delivery,
    ...
    log.info(
      `Fleetbase delivery status update processed: order=${order.id}, status=${status} → ${medusaStatus}`,
    );
  } catch (error) {
    log.error("Failed to process Fleetbase delivery status update", {
...
async function handleDriverAssigned(payload: any, container: any) {
  ...
  try {
    ...
    if (!order) {
      log.info(
        `No matching order found for Fleetbase driver assignment, order_id=${order_id}`,
      );
      return;
    }
    ...
    log.info(
      `Fleetbase driver assigned: order=${order.id}, driver=${driver_name}`,
    );
  } catch (error) {
    log.error("Failed to process Fleetbase driver assignment", {
      error: error instanceof Error ? (error instanceof Error ? error.message : String(error)) : error,
    });
  }
}
...
async function handleRouteUpdated(payload: any, container: any) {
  ...
  try {
    ...
    if (!order) {
      log.info(
        `No matching order found for Fleetbase route update, order_id=${order_id}`,
      );
      return;
    }
    ...
    log.info(
      `Fleetbase route updated: order=${order.id}, new_eta=${new_eta}, route=${route_id}`,
    );
  } catch (error) {
    log.error("Failed to process Fleetbase route update", {
      error: error instanceof Error ? (error instanceof Error ? error.message : String(error)) : error,
    });
  }
}
...
async function handleDeliveryCompleted(payload: any, container: any) {
  ...
  try {
    ...
    const completed_at =
      payload.completed_at || payload.delivered_at || new Date().toISOString();
    ...
    if (!order) {
      log.info(
        `No matching order found for Fleetbase delivery completion, order_id=${order_id}`,
      );
      return;
    }
    ...
    try {
      ...
      await dispatchEventToTemporal(
        "fulfillment.delivery_confirmed",
        {
          order_id: order.id,
          completed_at,
      ...
      log.info(
        `Post-delivery workflow dispatch skipped: ${dispatchErr instanceof Error ? dispatchErr.message : dispatchErr}`,
      );
    }

    log.info(
      `Fleetbase delivery completed: order=${order.id}, completed_at=${completed_at}`,
    );
  } catch (error) {
    log.error("Failed to process Fleetbase delivery completion", {
      error: error instanceof Error ? (error instanceof Error ? error.message : String(error)) : error,
    });
  }
}
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    if (secret) {
      const apiKey =
        (req.headers["x-fleetbase-key"] as string) ||
        (req.headers["x-fleetbase-signature"] as string);
      if (!apiKey) {
        logger.info(
          `[Webhook:Fleetbase] Missing API key header (correlation: ${correlationId})`,
        );
      ...
      if (apiKey !== secret) {
        logger.info(
          `[Webhook:Fleetbase] Invalid API key (correlation: ${correlationId})`,
        );
    ...
    logger.info(
      `[Webhook:Fleetbase] Received event: ${event} (correlation: ${correlationId})`,
    );
    ...
    switch (event) {
      ...
      default:
        logger.info(
          `[Webhook:Fleetbase] Unhandled event: ${event} (correlation: ${correlationId})`,
        );
        break;
    }
    ...
    try {
      ...
      logger.info(
        `[Webhook:Fleetbase] Temporal dispatch skipped: ${(error instanceof Error ? error.message : String(error))}`,
      );
    }
