...
class RealEstateModuleService extends MedusaService({
  ...
  async publishProperty(propertyId: string): Promise<any> {
    ...
    return await this.updatePropertyListings({
      id: propertyId,
      status: "published",
  ...
  async scheduleViewing(
    ...
    return await this.createViewingAppointments({
      property_listing_id: propertyId,
      viewer_id: viewerId,
      scheduled_date: date,
      status: "scheduled",
  ...
  async makeOffer(
    ...
    const valuation = await this.createPropertyValuations({
      property_listing_id: propertyId,
      buyer_id: buyerId,
      ...
      difference_percentage: property.price
        ? Math.round(
            ((amount - Number(property.price)) / Number(property.price)) *
              10000,
          ) / 100
        : 0,
  ...
  async calculateMortgage(
    ...
    const monthlyPayment =
      rate > 0
        ? (loanAmount * rate * Math.pow(1 + rate, payments)) /
  ...
  async createInquiry(
    ...
    return await this.createPropertyValuations({
      property_listing_id: propertyId,
      buyer_id: buyerId,
  ...
  async getMarketAnalysis(filters: {
    location?: string;
    propertyType?: string;
  }): Promise<{
    ...
    const prices = list
      .map((l: any) => Number(l.price || l.rent_price || 0))
      .filter((p: number) => p > 0)
      .sort((a: number, b: number) => a - b);
    const totalPrice = prices.reduce((sum: number, p: number) => sum + p, 0);
    const averagePrice =
      prices.length > 0 ? Math.round(totalPrice / prices.length) : 0;
    const medianPrice =
    ...
    const daysOnMarket = list
      ...
      .map((l: any) =>
        Math.floor(
          (now - new Date(l.published_at).getTime()) / (1000 * 60 * 60 * 24),
        ),
      );
    const averageDaysOnMarket =
      daysOnMarket.length > 0
        ? Math.round(
            daysOnMarket.reduce((s: number, d: number) => s + d, 0) /
              daysOnMarket.length,
          )
        : 0;
