...
class AuctionModuleService extends MedusaService({
  ...
  async placeBid(
    ...
    const bid = await this.createBids({
      tenant_id: "default",
    ...
    await this.updateAuctionListings({
      id: auctionId,
  ...
  async closeAuction(auctionId: string): Promise<any> {
    ...
    if (hasMetReserve && highestBid) {
      const result = await this.createAuctionResults({
        auction_id: auctionId,
  ...
  async getHighestBid(auctionId: string): Promise<any | null> {
    const bids = await this.listBids({
      auction_id: auctionId,
      status: "active",
    }) as any;
  ...
  async getAuctionWithBids(auctionId: string): Promise<any> {
    ...
    return {
      ...
      lowestBid:
        sortedBids.length > 0
          ? Number(sortedBids[sortedBids.length - 1].amount)
          : null,
    };
  }

  async checkAntiSniping(
    auctionId: string,
    bidTime: Date,
  ): Promise<{ extended: boolean; newEndTime?: Date }> {
    const auction = await this.retrieveAuctionListing(auctionId) as any;
  ...
  async validateBidIncrement(
    auctionId: string,
    bidAmount: number,
  ): Promise<{ valid: boolean; minimumBid: number; currentPrice: number }> {
    const auction = await this.retrieveAuctionListing(auctionId) as any;
    ...
    const minimumBid =
      Math.round((currentPrice + minimumIncrement) * 100) / 100;
