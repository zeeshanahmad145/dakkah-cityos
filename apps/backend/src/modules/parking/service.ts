...
class ParkingModuleService extends MedusaService({
  ...
  async findAvailableSpots(
    ...
    const zone = await this.retrieveParkingZone(zoneId) as any;
  ...
  async reserveSpot(
    ...
    const session = await this.createParkingSessions({
      zone_id: spotId,
  ...
  async calculateFee(
    spotId: string,
    duration: number,
  ): Promise<{ hourlyRate: number; totalFee: number }> {
    const zone = await this.retrieveParkingZone(spotId) as any;
  ...
  async releaseSpot(spotId: string): Promise<any> {
    ...
    return await this.updateParkingSessions({
      id: sessionList[0].id,
      status: "completed",
      ended_at: new Date(),
    } as any);
  ...
  async checkOut(reservationId: string): Promise<{
    ...
    const zone = await this.retrieveParkingZone(session.zone_id) as any;
    const hourlyRate = Number(zone.hourly_rate || 5);
    const fee = Math.round(hourlyRate * durationHours * 100) / 100;

    await this.updateParkingSessions({
      id: reservationId,
      status: "completed",
  ...
  async calculateDynamicFee(
    ...
    const totalFee =
      Math.round(baseRate * vehicleMultiplier * hours * 100) / 100;
