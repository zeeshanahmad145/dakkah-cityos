...
class CharityModuleService extends MedusaService({
  ...
  async processDonation(
    ...
    const donation = await this.createDonations({
    ...
    await this.updateDonationCampaigns({
      id: campaignId,
  ...
  async generateImpactReport(campaignId: string): Promise<any> {
    ...
    const avgDonation =
      donationList.length > 0 ? totalAmount / donationList.length : 0;

    const report = await this.createImpactReports({
      campaign_id: campaignId,
  ...
  async createCampaign(data: {
    ...
    if (org.status !== "active" && org.status !== "verified") {
      throw new Error(
        "Organization must be active or verified to create campaigns",
      );
    }

    const campaign = await this.createDonationCampaigns({
  ...
  async issueTaxReceipt(donationId: string): Promise<{
    ...
    const campaign = (await this.retrieveDonationCampaign(
  ...
  async calculateTaxDeduction(
    ...
    const countryRules: Record<
      string,
      { percentage: number; maxDeductible: number | null }
    > = {
    ...
    let deductibleAmount =
      Math.round(((donationAmount * rules.percentage) / 100) * 100) / 100;
