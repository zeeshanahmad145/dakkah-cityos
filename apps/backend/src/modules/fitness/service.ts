...
class FitnessModuleService extends MedusaService({
  ...
  async bookClass(classId: string, customerId: string): Promise<any> {
    const schedule = await this.retrieveClassSchedule(classId) as any;
    const existingBookings = await this.listClassBookings({
      schedule_id: classId,
    }) as any;
    const bookingList = Array.isArray(existingBookings)
      ? existingBookings
      : [existingBookings].filter(Boolean);
    if (bookingList.length >= (schedule.max_capacity || 20)) {
      throw new Error("Class is fully booked");
    }
    const alreadyBooked = bookingList.find(
      (b: any) => b.customer_id === customerId && b.status === "booked",
    );
    if (alreadyBooked) {
      throw new Error("You have already booked this class");
    }
    return await this.createClassBookings({
      tenant_id: "default",
      schedule_id: classId,
  ...
  async cancelBooking(bookingId: string): Promise<any> {
    ...
    return await this.updateClassBookings({
      id: bookingId,
      status: "cancelled",
  ...
  async getSchedule(trainerId: string, date: Date): Promise<any[]> {
    const schedules = await this.listClassSchedules({
    ...
    return list
      .filter((s: any) => new Date(s.start_time).toDateString() === targetDate)
      .sort(
        (a: any, b: any) =>
  ...
  async createMembership(
    ...
    return await this.createGymMemberships({
      tenant_id: "default",
      customer_id: customerId,
  ...
  async checkMembershipStatus(customerId: string): Promise<{
    ...
    const memberships = await this.listGymMemberships({
      customer_id: customerId,
    }) as any;
    ...
    const sorted = list.sort(
      (a: any, b: any) =>
        new Date(b.end_date || b.created_at).getTime() -
        new Date(a.end_date || a.created_at).getTime(),
    );
  ...
  async getClassAvailability(classScheduleId: string): Promise<{
    ...
    const schedule = await this.retrieveClassSchedule(classScheduleId) as any;
    ...
    const bookingList = Array.isArray(bookings)
      ? bookings
      : [bookings].filter(Boolean);
    const booked = bookingList.filter((b: any) =>
      ["booked", "checked_in", "completed"].includes(b.status),
    ).length;
  ...
  async cancelClassBooking(
    ...
    await this.updateClassBookings({
      id: bookingId,
      status: "cancelled",
      cancelled_at: new Date(),
    } as any);
  ...
  async recordWorkout(
    ...
    return await this.createWellnessPlans({
      tenant_id: "default",
      customer_id: customerId,
