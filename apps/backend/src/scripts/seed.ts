...
export default async function seedCoreData({ container }: ExecArgs) {
  ...
  const link = container.resolve(ContainerRegistrationKeys.LINK) as unknown as any;
  ...
  const storeModuleService = container.resolve(Modules.STORE) as unknown as any;
  ...
  try {
    ...
    if (!existingUsers || existingUsers.length === 0) {
      ...
      const userModuleService = container.resolve(Modules.USER) as unknown as any;
    ...
    logger.warn(`  Admin user creation skipped: ${(error instanceof Error ? error.message : String(error))}`);
  }
  ...
  const SEED_CUSTOMERS = [
    ...
    {
      email: "fatima@dakkah.com",
      first_name: "Fatima",
      last_name: "Al-Saud",
      phone: "+966502345678",
      address: {
        address_1: "Prince Sultan Road, Al Zahra District",
        city: "Jeddah",
        postal_code: "23425",
      },
    },
    {
      email: "ahmed@dakkah.com",
      first_name: "Ahmed",
      last_name: "Hassan",
      phone: "+966503456789",
      address: {
        address_1: "King Abdulaziz Road, Al Khobar",
        city: "Dammam",
        postal_code: "31952",
      },
    },
    {
      email: "khalid@dakkah.com",
      first_name: "Khalid",
      last_name: "Al-Dakkah",
      phone: "+966504567890",
      address: {
        address_1: "Tahlia Street, Al Rawdah District",
        city: "Jeddah",
        postal_code: "23432",
      },
    },
    {
      email: "sara@dakkah.com",
      first_name: "Sara",
      last_name: "Al-Mansour",
      phone: "+966505678901",
      address: {
        address_1: "Northern Ring Road, Al Nakheel",
        city: "Riyadh",
        postal_code: "12382",
      },
    },
    {
      email: "omar@dakkah.com",
      first_name: "Omar",
      last_name: "Al-Otaibi",
      phone: "+966506789012",
      address: {
        address_1: "Corniche Road, Al Hamra",
        city: "Jeddah",
        postal_code: "23511",
      },
    },
    {
      email: "noura@dakkah.com",
      first_name: "Noura",
      last_name: "Al-Shamrani",
      phone: "+966507890123",
      address: {
        address_1: "King Abdullah Road, Al Malaz",
        city: "Riyadh",
        postal_code: "12631",
      },
    },
    {
      email: "abdullah@dakkah.com",
      first_name: "Abdullah",
      last_name: "Al-Qahtani",
      phone: "+966508901234",
      address: {
        address_1: "Prince Faisal Road, Al Murjan",
        city: "Dammam",
        postal_code: "32411",
      },
    },
    {
      email: "test_customer@dakkah.com",
      first_name: "Test",
      last_name: "Customer",
      phone: "+966509012345",
      address: {
        address_1: "King Saud Road, Al Wizarat",
        city: "Riyadh",
        postal_code: "12611",
      },
    },
  ...
  try {
    const authModuleService = container.resolve(Modules.AUTH) as unknown as any;
    const customerModuleService = container.resolve(Modules.CUSTOMER) as unknown as any;
    ...
    logger.warn(`  Customer seeding skipped: ${(error instanceof Error ? error.message : String(error))}`);
  }
  ...
  if (pricePreferences.length > 0) {
    ...
    await (container.resolve(Modules.PRICING) as unknown as any).deletePricePreferences(
      ids,
    );
  }
  ...
  if (!existingRegions || existingRegions.length === 0) {
    ...
    if (menaRegion) {
      ...
      logger.info(
        `  MENA region not found, using fallback: ${existingRegions[0].name} (${existingRegions[0].id})`,
      );
    }
  ...
  const stockLocationModule = container.resolve(Modules.STOCK_LOCATION) as unknown as any;
  ...
  if (existingLocations.length === 0) {
    ...
    try {
      ...
      logger.warn(
        `  Fulfillment provider link already exists or failed: ${linkError.message}`,
      );
    }
  } else {
  ...
  if (existingFulfillmentSets.length === 0) {
    const fulfillmentSet = await fulfillmentModuleService.createFulfillmentSets(
      {
        ...
        service_zones: [
          {
            name: "MENA Zone",
            geo_zones: ["sa", "ae", "kw", "qa", "bh", "om"].map(
              (country_code) => ({
                country_code,
    ...
    try {
      ...
      logger.warn(
        `  Fulfillment set link already exists or failed: ${linkError.message}`,
      );
    }
  ...
  try {
    ...
    logger.warn(
      `  Sales channel linking skipped (may already be linked): ${linkError.message}`,
    );
  }
  ...
  try {
    ...
    logger.warn(
      `  Sales channel to API key linking skipped: ${linkError.message}`,
    );
  }
