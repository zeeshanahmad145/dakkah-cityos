  ...
  async (input: { payments: any[]; orders: any[] }) => {
    ...
    for (const payment of input.payments) {
      const match = input.orders.find((o: any) => {
        ...
        const amountMatch = Math.abs((o.total || 0) - (payment.amount || 0)) < 0.01
        const referenceMatch = o.id === payment.reference_id
        const dateProximity = Math.abs(
          new Date(o.created_at).getTime() - new Date(payment.created_at).getTime()
        ) < 86400000
      ...
      if (match) {
        ...
        unmatched.push({
          ...
          matched: false,
          reason: "No matching order found by amount, reference, or date proximity",
        })
      }
    }
...
const reconcileStep = createStep(
  ...
  async (input: { matched: any[]; unmatchedCount: number; tenantId: string }) => {
    const reconciliation = {
      ...
      high_confidence: input.matched.filter((m: any) => m.confidence === 1.0).length,
      low_confidence: input.matched.filter((m: any) => m.confidence < 1.0).length,
...
const generateReportStep = createStep(
  ...
  async (input: { reconciliation: any; unmatched: any[]; dateFrom: string; dateTo: string }) => {
    const report = {
...
export const paymentReconciliationWorkflow = createWorkflow(
  ...
  (input: PaymentReconciliationInput) => {
    ...
    const { orders } = fetchOrderRecordsStep({ dateFrom: input.dateFrom, dateTo: input.dateTo })
    const { matched, unmatched, unmatchedCount } = matchTransactionsStep({ payments, orders })
    const { reconciliation } = reconcileStep({ matched, unmatchedCount, tenantId: input.tenantId })
    const { report } = generateReportStep({ reconciliation, unmatched, dateFrom: input.dateFrom, dateTo: input.dateTo })
