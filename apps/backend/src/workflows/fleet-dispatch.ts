...
const prepareOrderForDispatchStep = createStep(
  ...
  async (
    compensationData: { dispatchRequest: any } | undefined,
    { container },
  ) => {
    ...
    try {
      ...
      if (compensationData.dispatchRequest.id) {
        await fulfillmentModule.cancelFulfillment(
          compensationData.dispatchRequest.id,
        );
      }
    } catch (error) {}
  },
...
const findAvailableDriverStep = createStep(
  ...
  async (
    input: { pickupAddress: string; priority: string; vehicle_type?: string },
    { container },
  ) => {
    try {
      const fleetbaseService = container.resolve("fleetbaseService") as unknown as any;
...
const assignDriverStep = createStep(
  "assign-driver-step",
  async (
    input: { orderId: string; driverId: string; source?: string },
    { container },
  ) => {
      ...
      const assignment = {
        ...
        assignment_error:
          error instanceof Error
            ...
            : "Failed to assign driver via FleetbaseService",
        assigned_at: new Date(),
      };
      return new StepResponse({ assignment }, { assignment });
    }
    ...
    try {
      ...
      if (compensationData.assignment.id) {
        await fulfillmentModule.updateFulfillment(
          compensationData.assignment.id,
          {
            status: "unassigned",
            driver_id: null,
          },
  ...
  async (compensationData: { tracking: any } | undefined, { container }) => {
    ...
    try {
      ...
      if (compensationData.tracking.tracking_id) {
        await fulfillmentModule.cancelFulfillment(
          compensationData.tracking.tracking_id,
        );
      }
    } catch (error) {}
  },
...
export const fleetDispatchWorkflow = createWorkflow(
  ...
  (input: FleetDispatchInput) => {
    ...
    const driverResult = findAvailableDriverStep({
    ...
    const { assignment } = assignDriverStep({
      ...
      driverId: driverResult.driver.driver_id,
      source: driverResult.driver.source,
    });
    const { tracking } = initializeTrackingStep({
      orderId: input.orderId,
      driverId: driverResult.driver.driver_id,
    });
