...
const creditPointsStep = createStep(
  ...
  async (
    ...
    const loyaltyModule = container.resolve("loyalty") as unknown as any;
  ...
  async (
    compensationData: { transactionId: string } | undefined,
    { container },
  ) => {
    if (!compensationData?.transactionId) return;
    try {
      const loyaltyModule = container.resolve("loyalty") as unknown as any;
      await loyaltyModule.deleteLoyaltyTransactions(
        compensationData.transactionId,
      );
    } catch (error) {}
  },
);
...
export const loyaltyRewardWorkflow = createWorkflow(
  ...
  (input: LoyaltyRewardInput) => {
    ...
    const { transaction } = creditPointsStep({
      customerId: input.customerId,
      points,
      orderId: input.orderId,
    });
    const notification = notifyRewardStep({
      customerId: input.customerId,
      points,
    });
