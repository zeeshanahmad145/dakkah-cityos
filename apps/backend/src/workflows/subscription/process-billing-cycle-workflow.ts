...
const loadBillingCycleStep = createStep(
  ...
  async (input: ProcessBillingCycleInput, { container }) => {
    ...
    if (cycle.status !== "upcoming") {
      throw new Error(
        `Billing cycle ${input.billing_cycle_id} is not in upcoming status`,
      );
    }
...
const markCycleProcessingStep = createStep(
  ...
  async ({ cycle }: { cycle: any }, { container }) => {
    ...
    return new StepResponse(
      ...
      { cycleId: cycle.id, previousStatus, previousAttemptCount },
    );
  },
  async (
    compensationData:
...
const createOrderFromSubscriptionStep = createStep(
  ...
  async (
    {
      ...
      subscription,
      items,
    }: { cycle: any; subscription: any; items: any[] },
    { container },
  ) => {
    const query = container.resolve(ContainerRegistrationKeys.QUERY) as unknown as any;
  ...
  async (compensationData: { cart: any } | undefined, { container }) => {
    ...
    try {
      const cartModule = container.resolve("cart") as unknown as any;
...
const processSubscriptionPaymentStep = createStep(
  ...
  async (
    {
      ...
      subscription,
      customer,
    }: { cart: any; subscription: any; customer: any },
    { container },
  ) => {
    ...
    if (!stripeSecretKey) {
      logger.warn(
        "STRIPE_SECRET_KEY not configured - marking payment as simulated",
      );
      return new StepResponse(
        {
          payment_status: "paid",
    ...
    if (!stripeCustomerId || !paymentMethodId) {
      logger.warn(
        `Missing payment details for subscription ${subscription.id}`,
      );
      return new StepResponse(
        {
    ...
    try {
      ...
      if (paymentIntent.status === "succeeded") {
        ...
        logger.warn(
          `Payment status ${paymentIntent.status} for subscription ${subscription.id}`,
        );
        return new StepResponse(
          {
      ...
      logger.error(
        `Payment failed for subscription ${subscription.id}: ${(error instanceof Error ? error.message : String(error))}`,
      );
  ...
  async (
    compensationData: { payment_id: string | null } | undefined,
    { container },
  ) => {
    ...
    try {
      ...
      await stripe.refunds.create({
        payment_intent: compensationData.payment_id,
      });
    } catch (error) {}
  },
);
...
export const processBillingCycleWorkflow = createWorkflow(
  ...
  (input: ProcessBillingCycleInput) => {
    ...
    const paymentInput = transform(
      { orderResult, loadResult },
      ({ orderResult, loadResult }) => ({
    ...
    const completeInput = transform(
      { markResult, orderResult, paymentResult, loadResult },
      ({ markResult, orderResult, paymentResult, loadResult }) => ({
        cycle: markResult.updatedCycle,
        cart: orderResult.cart,
