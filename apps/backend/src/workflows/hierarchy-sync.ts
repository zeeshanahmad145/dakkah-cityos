...
const detectChangeStep = createStep(
  ...
  async (input: HierarchySyncInput, { container }) => {
    const nodeModule = container.resolve("node") as unknown as any;
...
const propagateChangeStep = createStep(
  ...
  async (
    input: { nodeId: string; changeType: string; parentId?: string },
    { container },
  ) => {
    const nodeModule = container.resolve("node") as unknown as any;
    const affectedNodes = await nodeModule.listNodes({
...
const auditHierarchyChangeStep = createStep(
  ...
  async (
    input: { nodeId: string; changeType: string; tenantId: string },
    { container },
  ) => {
    const auditModule = container.resolve("audit") as unknown as any;
  ...
  async (
    compensationData: { auditEntryId: string } | undefined,
    { container },
  ) => {
...
export const hierarchySyncWorkflow = createWorkflow(
  ...
  (input: HierarchySyncInput) => {
    ...
    const { validation } = validateHierarchyStep({
      nodeId: input.nodeId,
      parentId: input.parentId,
      changeType: input.changeType,
    });
    const { propagation } = propagateChangeStep({
      nodeId: input.nodeId,
      changeType: input.changeType,
      parentId: input.parentId,
    });
    const { auditEntry } = auditHierarchyChangeStep({
      nodeId: input.nodeId,
      changeType: input.changeType,
