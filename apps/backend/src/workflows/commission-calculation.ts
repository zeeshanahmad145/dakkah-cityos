...
const calculateCommissionStep = createStep(
  ...
  async (input: CommissionCalcInput, { container }) => {
    ...
    const rules = await commissionModule.listCommissionRules({
...
const deductCommissionStep = createStep(
  ...
  async (
    input: { vendorId: string; commissionAmount: number; orderId: string },
    { container },
  ) => {
    const commissionModule = container.resolve("commission") as unknown as any;
  ...
  async (compensationData: { transaction: any } | undefined, { container }) => {
    ...
    try {
      const commissionModule = container.resolve("commission") as unknown as any;
      await commissionModule.deleteCommissionTransactions(
        compensationData.transaction.id,
      );
    } catch (error) {}
  },
);
...
const recordPayoutStep = createStep(
  ...
  async (
    input: { vendorId: string; netAmount: number; orderId: string },
    { container },
  ) => {
...
export const commissionCalculationWorkflow = createWorkflow(
  ...
  (input: CommissionCalcInput) => {
    const { commissionAmount, rate, netAmount } =
      calculateCommissionStep(input);
    const { transaction } = deductCommissionStep({
      vendorId: input.vendorId,
      commissionAmount,
      orderId: input.orderId,
    });
    const { payout } = recordPayoutStep({
      vendorId: input.vendorId,
      netAmount,
      orderId: input.orderId,
    });
