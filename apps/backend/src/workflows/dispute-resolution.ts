...
const openDisputeStep = createStep(
  ...
  async (input: DisputeResolutionInput, { container }) => {
    const disputeModule = container.resolve("dispute") as unknown as any;
  ...
  async (
    compensationData: { disputeId: string } | undefined,
    { container },
  ) => {
...
const reviewDisputeStep = createStep(
  ...
  async (input: { disputeId: string }, { container }) => {
    ...
    return new StepResponse(
      { dispute: updated },
      { disputeId: input.disputeId, previousStatus: "open" },
    );
  },
  async (
    compensationData: { disputeId: string; previousStatus: string } | undefined,
    { container },
  ) => {
    if (!compensationData?.disputeId) return;
    try {
...
const resolveDisputeStep = createStep(
  ...
  async (input: { disputeId: string; resolution: string }, { container }) => {
    ...
    return new StepResponse(
      { dispute: resolved },
      { disputeId: input.disputeId, previousStatus: "under_review" },
    );
  },
  async (
    compensationData: { disputeId: string; previousStatus: string } | undefined,
