...
const createAuctionStep = createStep(
  ...
  async (input: AuctionLifecycleInput, { container }) => {
    ...
    const auction = await auctionModule.createAuctions({
      ...
      minimum_increment: input.minimumIncrement || Math.max(1, Math.round(input.startingPrice * 0.05)),
...
const openAuctionStep = createStep(
  ...
  async (input: { auctionId: string; startTime: string }, { container }) => {
    ...
    if (startTime > now) {
      ...
      return new StepResponse({ auction: updated }, { auctionId: input.auctionId, previousStatus: "draft" })
    }
...
const closeAuctionStep = createStep(
  "close-auction-step",
  async (input: { auctionId: string; reservePrice?: number; startingPrice: number; minimumIncrement?: number }, { container }) => {
    const auctionModule = container.resolve("auction") as any
    ...
    const sortedBids = bids.sort((a: any, b: any) => (b.amount || 0) - (a.amount || 0))
    const highestBid = sortedBids.length > 0 ? sortedBids[0] : null
    const reservePrice = input.reservePrice || input.startingPrice
    const minimumIncrement = input.minimumIncrement || Math.max(1, Math.round(input.startingPrice * 0.05))
    ...
    const invalidBids = sortedBids.filter((bid: any, index: number) => {
      ...
      if (previousBid && (bid.amount - previousBid.amount) < minimumIncrement) return true
      return false
    })
...
export const auctionLifecycleWorkflow = createWorkflow(
  ...
  (input: AuctionLifecycleInput) => {
    ...
    const opened = openAuctionStep({ auctionId: auction.id, startTime: input.startTime })
