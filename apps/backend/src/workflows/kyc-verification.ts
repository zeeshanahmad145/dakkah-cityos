...
type KycVerificationInput = {
  ...
  dateOfBirth: string
  country: string
  id_document?: { type?: string; number?: string; expiry?: string; size?: number }
  proof_of_address?: { type?: string; issued_date?: string; size?: number }
  business_registration?: { registration_number?: string; registration_date?: string; size?: number }
...
const verifyDocumentsStep = createStep(
  ...
  async (input: { vendorId: string; documentUrl: string; documentType: string; id_document?: any; proof_of_address?: any; business_registration?: any }) => {
    const reasons: string[] = []

    if (!input.id_document || !input.id_document.type || !input.id_document.number) {
      reasons.push("ID document is missing or incomplete (type and number required)")
    } else {
      if (!VALID_DOC_TYPES.includes(input.id_document.type)) {
        reasons.push(`Invalid ID document type '${input.id_document.type}'. Must be one of: ${VALID_DOC_TYPES.join(", ")}`)
      }
      if (input.id_document.size && input.id_document.size > MAX_DOC_SIZE_BYTES) {
        reasons.push("ID document exceeds maximum file size of 10MB")
      }
      if (input.id_document.expiry && new Date(input.id_document.expiry) < new Date()) {
    ...
    if (!input.proof_of_address || !input.proof_of_address.type) {
      ...
      if (!VALID_ADDRESS_TYPES.includes(input.proof_of_address.type)) {
        reasons.push(`Invalid proof of address type '${input.proof_of_address.type}'. Must be one of: ${VALID_ADDRESS_TYPES.join(", ")}`)
      }
      if (input.proof_of_address.size && input.proof_of_address.size > MAX_DOC_SIZE_BYTES) {
        reasons.push("Proof of address document exceeds maximum file size of 10MB")
      }
    ...
    if (!input.business_registration || !input.business_registration.registration_number) {
      reasons.push("Business registration document is missing or incomplete (registration number required)")
    } else {
      if (input.business_registration.size && input.business_registration.size > MAX_DOC_SIZE_BYTES) {
        reasons.push("Business registration document exceeds maximum file size of 10MB")
      }
    }
...
const scoreApplicantStep = createStep(
  ...
  async (input: { vendorId: string; verification: any; business_start_date?: string }) => {
    let score = 0

    const hasIdDoc = input.verification.document_valid || !input.verification.reasons?.some((r: string) => r.toLowerCase().includes("id document"))
    const hasAddress = input.verification.document_valid || !input.verification.reasons?.some((r: string) => r.toLowerCase().includes("proof of address"))
    const hasBusiness = input.verification.document_valid || !input.verification.reasons?.some((r: string) => r.toLowerCase().includes("business registration"))
    ...
    if (input.business_start_date) {
      const businessAge = (Date.now() - new Date(input.business_start_date).getTime()) / (1000 * 60 * 60 * 24 * 365)
...
const decideKycStep = createStep(
  ...
  async (input: { vendorId: string; score: number; riskLevel: string; status: string }, { container }) => {
...
export const kycVerificationWorkflow = createWorkflow(
  ...
  (input: KycVerificationInput) => {
    ...
    const { decision } = decideKycStep({ vendorId: input.vendorId, score, riskLevel, status })
