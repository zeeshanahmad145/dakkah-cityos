...
export function sanitizeImageUrls<T extends Record<string, any>>(
  ...
  for (const field of imageFields) {
    ...
    if (typeof val === "string" && isUnsplashUrl(val)) {
      result[field] = replaceUrl(val, vertical, index);
    } else if (Array.isArray(val)) {
      result[field] = val.map((v: any, i: number) =>
        typeof v === "string" && isUnsplashUrl(v)
          ? replaceUrl(v, vertical, index + i)
          : v,
      );
    }
  }
...
export function sanitizeList<T extends Record<string, any>>(
  ...
  return items.map((item, i) =>
    sanitizeImageUrls(item, vertical, imageFields, i),
  );
}
