...
} from "../../../src/api/store/advertising/route";
import {
  GET as affiliatesGET,
  POST as affiliatesPOST,
} from "../../../src/api/store/affiliates/route";
import {
  GET as auctionsGET,
  POST as auctionsPOST,
} from "../../../src/api/store/auctions/route";
...
} from "../../../src/api/store/gift-cards/route";
import {
  GET as loyaltyGET,
  POST as loyaltyPOST,
} from "../../../src/api/store/loyalty/route";
import {
  GET as walletGET,
  POST as walletPOST,
} from "../../../src/api/store/wallet/route";
...
describe("Commerce Store Routes", () => {
  ...
  describe("Advertising /store/advertising", () => {
    const mockService = {
      listAdPlacements: jest.fn(),
      createAdPlacements: jest.fn(),
    };
    ...
    it("GET passes pagination params", async () => {
      ...
      const res = createRes();
      await advertisingGET(req, res);
      expect(mockService.listAdPlacements).toHaveBeenCalledWith(
        {},
        { skip: 10, take: 5 },
      );
    });
    ...
    it("GET passes filter parameters", async () => {
      ...
      const req = createReq({
        scope: { resolve: jest.fn(() => mockService) },
        query: { tenant_id: "t1", placement_type: "sidebar", status: "active" },
      });
      const res = createRes();
      await advertisingGET(req, res);
      expect(mockService.listAdPlacements).toHaveBeenCalledWith(
        { tenant_id: "t1", placement_type: "sidebar", status: "active" },
        expect.any(Object),
      );
    });
  ...
  describe("Affiliates /store/affiliates", () => {
    ...
    it("GET passes filters", async () => {
      ...
      const req = createReq({
        scope: { resolve: jest.fn(() => mockService) },
        query: { tenant_id: "t1", status: "active" },
      });
      const res = createRes();
      await affiliatesGET(req, res);
      expect(mockService.listAffiliates).toHaveBeenCalledWith(
  ...
  describe("Auctions /store/auctions", () => {
    ...
    it("GET returns active auctions", async () => {
      ...
      expect(mockService.listAuctionListings).toHaveBeenCalledWith(
  ...
  describe("Flash Sales /store/flash-sales", () => {
    ...
    it("GET returns flash sales", async () => {
      ...
      mockQuery.graph.mockResolvedValue({
        data: [{ id: "p_1", status: "active" }],
      });
      const req = createReq({
        scope: {
          resolve: jest.fn((name: string) =>
            name === "query" ? mockQuery : mockPromotionExt,
          ),
        },
      });
      const res = createRes();
      ...
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({ flash_sales: expect.any(Array) }),
      );
    });
  ...
  describe("Gift Cards /store/gift-cards", () => {
    ...
    it("GET returns active gift cards", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({ gift_cards: expect.any(Array) }),
      );
    });
  ...
  describe("Wallet /store/wallet", () => {
    ...
    it("GET returns wallet balances", async () => {
      mockLoyalty.listLoyaltyAccounts.mockResolvedValue([
        { points_balance: 100 },
      ]);
      ...
      const req = createReq({
        scope: {
          resolve: jest.fn((name: string) =>
            name === "loyalty" ? mockLoyalty : mockPromotionExt,
          ),
        },
        auth_context: { actor_id: "cust_1" },
      });
