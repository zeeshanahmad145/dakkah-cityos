...
describe("WalletModuleService", () => {
  ...
  describe("createWallet", () => {
    it("creates a wallet for a new customer", async () => {
      jest.spyOn(service, "listWallets").mockResolvedValue([] as any)
    ...
    it("defaults currency to usd", async () => {
      jest.spyOn(service, "listWallets").mockResolvedValue([] as any)
      const createSpy = jest.spyOn(service as any, "createWallets").mockResolvedValue({ id: "wal_1" })
    ...
    it("throws when wallet already exists for customer and currency", async () => {
      jest.spyOn(service, "listWallets").mockResolvedValue([{ id: "wal_existing" }] as any)
  ...
  describe("creditWallet", () => {
    it("credits wallet and creates transaction", async () => {
      ...
      expect(txSpy).toHaveBeenCalledWith(
        expect.objectContaining({ type: "credit", amount: 50, balance_after: 150 })
      )
    })
  ...
  describe("debitWallet", () => {
    it("debits wallet and creates transaction", async () => {
      ...
      expect(txSpy).toHaveBeenCalledWith(
        expect.objectContaining({ type: "debit", amount: -75, balance_after: 125 })
      )
    })
  ...
  describe("transferBetweenWallets", () => {
    it("transfers funds between two active wallets", async () => {
      jest.spyOn(service, "retrieveWallet")
        .mockResolvedValueOnce({ id: "wal_from", balance: 500, status: "active" } as any)
        .mockResolvedValueOnce({ id: "wal_to", balance: 100, status: "active" } as any)

      jest.spyOn(service, "debitWallet").mockResolvedValue({ id: "tx_debit" })
      jest.spyOn(service, "creditWallet").mockResolvedValue({ id: "tx_credit" })

      const result = await service.transferBetweenWallets("wal_from", "wal_to", 200)

      expect(result.amount).toBe(200)
      expect(service.debitWallet).toHaveBeenCalledWith("wal_from", 200, expect.any(String), undefined)
      expect(service.creditWallet).toHaveBeenCalledWith("wal_to", 200, expect.any(String), undefined)
    })
    ...
    it("throws when source wallet has insufficient balance", async () => {
      jest.spyOn(service, "retrieveWallet")
        .mockResolvedValueOnce({ id: "wal_from", balance: 50, status: "active" } as any)
        .mockResolvedValueOnce({ id: "wal_to", balance: 100, status: "active" } as any)
  ...
  describe("getTransactionHistory", () => {
    ...
    it("applies custom limit and offset", async () => {
      jest.spyOn(service, "listWalletTransactions").mockResolvedValue([] as any)
