...
describe("RestaurantModuleService", () => {
  ...
  describe("getMenuItems", () => {
    it("should return available menu items for a restaurant", async () => {
      jest.spyOn(service, "listMenuItems" as any).mockResolvedValue([
    ...
    it("should filter by category when provided", async () => {
      const listSpy = jest.spyOn(service, "listMenuItems" as any).mockResolvedValue([
        { id: "mi_01", name: "Burger", category: "main" },
      ])
  ...
  describe("placeOrder", () => {
    it("should create a kitchen order with total amount", async () => {
      jest.spyOn(service, "retrieveRestaurant" as any).mockResolvedValue({ id: "rest_01" })
      jest.spyOn(service, "retrieveMenuItem" as any)
        .mockResolvedValueOnce({ id: "mi_01", price: 1500 })
        .mockResolvedValueOnce({ id: "mi_02", price: 500 })
      jest.spyOn(service, "createKitchenOrders" as any).mockImplementation(async (data: any) => ({
        id: "ord_01", ...data,
      }))
  ...
  describe("updateOrderStatus", () => {
    it("should transition from pending to confirmed", async () => {
      jest.spyOn(service, "retrieveKitchenOrder" as any).mockResolvedValue({
        id: "ord_01", status: "pending",
      })
      jest.spyOn(service, "updateKitchenOrders" as any).mockResolvedValue({
        id: "ord_01", status: "confirmed",
      })
    ...
    it("should reject invalid state transition", async () => {
      ...
      await expect(service.updateOrderStatus("ord_01", "delivered")).rejects.toThrow(
    ...
    it("should reject transition from preparing to cancelled", async () => {
      ...
      await expect(service.updateOrderStatus("ord_01", "cancelled")).rejects.toThrow(
        "Cannot transition from preparing to cancelled"
      )
    })
  ...
  describe("calculateDeliveryFee", () => {
    it("should return a delivery fee and estimated time", async () => {
      jest.spyOn(service, "retrieveRestaurant" as any).mockResolvedValue({
        id: "rest_01", delivery_fee: 5,
      })

      const result = await service.calculateDeliveryFee("rest_01", "456 Oak Ave")
    ...
    it("should use default delivery fee when not set", async () => {
      ...
      const result = await service.calculateDeliveryFee("rest_01", "789 Pine St")
      expect(result.fee).toBe(5)
    })
  })
})
