  ...
  return {
    MedusaService: () =>
      class MockMedusaBase {
        async createCommissionRules(_data: any): Promise<any> {
          return {};
        }
        async listCommissionRules(_filter?: any, _opts?: any): Promise<any> {
          return [];
        }
        async createCommissionTransactions(_data: any): Promise<any> {
          return {};
        }
        async updateCommissionTransactions(
          _ids: any,
          _data?: any,
        ): Promise<any> {
          return {};
        }
        async listCommissionTransactions(
          _filter?: any,
          _opts?: any,
        ): Promise<any> {
          return [];
...
describe("CommissionModuleService", () => {
      ...
      jest.spyOn(service, "listCommissionRules").mockResolvedValue([
        {
          ...
          commission_flat_amount: null,
          priority: 1,
          tiers: [{ min_amount: 5000, max_amount: 10000, rate: 15 }],
        },
      ]);
  ...
  describe("processCommissionPayout", () => {
    it("processes payout for given transaction IDs", async () => {
      jest
        .spyOn(service, "listCommissionTransactions")
        .mockResolvedValue([{ id: "ctx_1" }, { id: "ctx_2" }]);

      const updateSpy = jest
        .spyOn(service, "updateCommissionTransactions")
        .mockResolvedValue({});
  ...
  describe("getTopEarningVendors", () => {
    it("returns vendors sorted by total commission", async () => {
      ...
      const result = await service.getTopEarningVendors({
        tenantId: "tenant_1",
        limit: 2,
      });
