...
describe("CharityModuleService", () => {
  ...
  describe("processDonation", () => {
    it("should process a valid donation", async () => {
      jest.spyOn(service, "retrieveDonationCampaign" as any).mockResolvedValue({
        id: "camp_01", status: "active", goal_amount: 100000,
        raised_amount: 50000, donor_count: 10,
      })
      jest.spyOn(service, "createDonations" as any).mockResolvedValue({
        id: "don_01", campaign_id: "camp_01", donor_id: "donor_01",
        amount: 5000, status: "completed",
      })
      jest.spyOn(service, "updateDonationCampaigns" as any).mockResolvedValue({})
    ...
    it("should reject donation to inactive campaign", async () => {
      jest.spyOn(service, "retrieveDonationCampaign" as any).mockResolvedValue({
        id: "camp_01", status: "completed", goal_amount: 100000,
      })

      await expect(service.processDonation("camp_01", "donor_01", 5000)).rejects.toThrow(
        "Campaign is not accepting donations"
      )
    })

    it("should reject zero amount donation", async () => {
      await expect(service.processDonation("camp_01", "donor_01", 0)).rejects.toThrow(
  ...
  describe("issueTaxReceipt", () => {
    it("should generate a tax receipt for a completed donation", async () => {
      jest.spyOn(service, "retrieveDonation" as any).mockResolvedValue({
        id: "don_01", amount: 5000, donor_id: "donor_01",
        campaign_id: "camp_01", status: "completed",
        donated_at: new Date(),
      })
      jest.spyOn(service, "retrieveDonationCampaign" as any).mockResolvedValue({
        id: "camp_01", title: "Clean Water", charity_org_id: "org_01",
      })
      jest.spyOn(service, "retrieveCharityOrg" as any).mockResolvedValue({
        id: "org_01", name: "Save the Children",
      })
  ...
  describe("calculateTaxDeduction", () => {
    it("should calculate US tax deduction at 60%", async () => {
      jest.spyOn(service, "retrieveDonation" as any).mockResolvedValue({
        id: "don_01", amount: 10000, status: "completed",
      })
    ...
    it("should calculate UK tax deduction at 100%", async () => {
      jest.spyOn(service, "retrieveDonation" as any).mockResolvedValue({
        id: "don_02", amount: 10000, status: "completed",
      })
    ...
    it("should return zero for non-deductible countries", async () => {
      jest.spyOn(service, "retrieveDonation" as any).mockResolvedValue({
        id: "don_03", amount: 10000, status: "completed",
      })
    ...
    it("should reject for non-completed donations", async () => {
      jest.spyOn(service, "retrieveDonation" as any).mockResolvedValue({
        id: "don_04", amount: 10000, status: "pending",
      })

      await expect(service.calculateTaxDeduction("don_04", "US")).rejects.toThrow(
