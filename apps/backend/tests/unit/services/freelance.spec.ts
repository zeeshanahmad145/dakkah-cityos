...
describe("FreelanceModuleService", () => {
  ...
  describe("submitProposal", () => {
    it("should submit a valid proposal", async () => {
      jest.spyOn(service, "retrieveGigListing" as any).mockResolvedValue({
        id: "gig_01", status: "open", budget: 5000,
      })
      jest.spyOn(service, "listProposals" as any).mockResolvedValue([])
      jest.spyOn(service, "createProposals" as any).mockResolvedValue({
        id: "prop_01", gig_listing_id: "gig_01", freelancer_id: "fl_01", status: "submitted",
      })
    ...
    it("should reject proposal on closed gig", async () => {
      jest.spyOn(service, "retrieveGigListing" as any).mockResolvedValue({
        id: "gig_01", status: "closed", budget: 5000,
      })
  ...
  describe("releaseMilestonePayment", () => {
    it("should release payment for approved milestone", async () => {
      jest.spyOn(service, "retrieveMilestone" as any).mockResolvedValue({
        id: "ms_01", status: "approved", amount: 2000, contract_id: "con_01",
      })
      jest.spyOn(service, "retrieveFreelanceContract" as any).mockResolvedValue({
        id: "con_01", freelancer_id: "fl_01", status: "active",
      })
      jest.spyOn(service, "updateMilestones" as any).mockResolvedValue({})
    ...
    it("should reject release for unapproved milestone", async () => {
      jest.spyOn(service, "retrieveMilestone" as any).mockResolvedValue({
        id: "ms_01", status: "pending", amount: 2000,
      })
    ...
    it("should reject release for already paid milestone", async () => {
      jest.spyOn(service, "retrieveMilestone" as any).mockResolvedValue({
        id: "ms_01", status: "paid", amount: 2000,
      })
