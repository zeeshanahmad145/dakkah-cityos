...
describe("ParkingModuleService", () => {
  ...
  describe("calculateParkingFee", () => {
    ...
    it("should reject zero duration (same entry/exit time)", async () => {
      ...
      await expect(
        service.calculateParkingFee(entry, entry, "hourly"),
      ).rejects.toThrow("Exit time must be after entry time");
    });
  ...
  describe("reserveSpotAdvanced", () => {
    it("should reserve a spot in an available zone", async () => {
      ...
      const result = await service.reserveSpotAdvanced(
        "zone_01",
        "car",
        futureDate,
        2,
    ...
    it("should reject when no spots available", async () => {
      ...
      jest
        .spyOn(service, "listParkingSessions")
        .mockResolvedValue(Array(5).fill({ status: "active" }));
