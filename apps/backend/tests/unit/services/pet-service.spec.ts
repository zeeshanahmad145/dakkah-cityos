...
describe("PetServiceModuleService", () => {
  ...
  describe("bookPetService", () => {
    it("should book a grooming service for a pet", async () => {
      ...
      const result = await service.bookPetService(
        "pet_01",
        "grooming",
        futureDate,
      );
    ...
    it("should book boarding for pet with valid vaccinations", async () => {
      ...
      jest
        .spyOn(service, "trackVaccinations")
        .mockResolvedValue([
    ...
    it("should reject boarding for pet with expired vaccinations", async () => {
      ...
      jest
        ...
        .mockResolvedValue([
          {
            vaccineName: "Rabies",
            nextDue: new Date("2024-01-01").toISOString(),
          },
        ]);
      ...
      await expect(
        service.bookPetService("pet_01", "boarding", futureDate),
      ).rejects.toThrow(
    ...
    it("should reject boarding for pet with no vaccinations", async () => {
      ...
      await expect(
        service.bookPetService("pet_02", "boarding", futureDate),
      ).rejects.toThrow(
        "Pet must have up-to-date vaccinations for boarding services",
      );
    });
    ...
    it("should reject booking without required fields", async () => {
      await expect(
        service.bookPetService("", "grooming", new Date()),
      ).rejects.toThrow("Pet ID and service type are required");
    });
  });
  ...
  describe("updateVaccinationRecord", () => {
    ...
    it("should reject record without vaccine name", async () => {
      await expect(
        service.updateVaccinationRecord("pet_01", {
          vaccineName: "",
          dateAdministered: new Date(),
        }),
