...
describe("GET /admin/wallets/:id", () => {
  ...
  it("requests transaction history with limit 50", async () => {
    await GET_ID(mockReq, mockRes)
    expect(mockWalletService.getTransactionHistory).toHaveBeenCalledWith("w1", { limit: 50 })
  })
...
describe("POST /admin/wallets/:id (credit/debit)", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    mockWalletService = {
      creditWallet: jest.fn().mockResolvedValue({ id: "tx1", type: "credit", amount: 50 }),
      debitWallet: jest.fn().mockResolvedValue({ id: "tx2", type: "debit", amount: 30 }),
    }
    mockReq = {
      params: { id: "w1" },
  ...
  it("credits wallet successfully", async () => {
    ...
    expect(mockWalletService.creditWallet).toHaveBeenCalledWith("w1", 50, "Top up", undefined)
    expect(mockRes.json).toHaveBeenCalledWith({
  ...
  it("debits wallet successfully", async () => {
    ...
    await POST_ID(mockReq, mockRes)
    expect(mockWalletService.debitWallet).toHaveBeenCalledWith("w1", 30, "Fee", undefined)
    expect(mockRes.json).toHaveBeenCalledWith({
  ...
  it("returns 400 on invalid type", async () => {
    ...
    expect(mockRes.json).toHaveBeenCalledWith({ message: "type must be 'credit' or 'debit'" })
  })
  ...
  it("returns 400 on zero amount", async () => {
    ...
    await POST_ID(mockReq, mockRes)
    expect(mockRes.status).toHaveBeenCalledWith(400)
    expect(mockRes.json).toHaveBeenCalledWith({ message: "amount must be positive" })
  })
  ...
  it("returns 400 on service error (insufficient balance)", async () => {
    ...
    mockWalletService.debitWallet.mockRejectedValue(new Error("Insufficient balance"))
    await POST_ID(mockReq, mockRes)
    expect(mockRes.status).toHaveBeenCalledWith(400)
  ...
  it("passes reference_id when provided", async () => {
    mockReq.body = { type: "credit", amount: 50, description: "Refund", reference_id: "ord_123" }
    await POST_ID(mockReq, mockRes)
    expect(mockWalletService.creditWallet).toHaveBeenCalledWith("w1", 50, "Refund", "ord_123")
  })
})
