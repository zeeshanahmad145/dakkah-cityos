...
describe("GET /admin/auctions", () => {
  ...
  it("returns auctions with pagination", async () => {
    ...
    expect(mockRes.json).toHaveBeenCalledWith({ auctions, count: 2, limit: 20, offset: 0 })
  })
  ...
  it("filters by status, seller_id, category_id", async () => {
    ...
    await GET(mockReq, mockRes)
    expect(mockModule.listAuctionListings).toHaveBeenCalledWith(
      expect.objectContaining({ status: "active", seller_id: "s1", category_id: "cat_1" }),
      expect.any(Object)
    )
  })
...
describe("POST /admin/auctions", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    mockModule = { createAuctionListings: jest.fn().mockResolvedValue({ id: "auc_new" }) }
    mockReq = {
      body: {
        title: "Rare Item",
        starting_price: 100,
  ...
  it("maps start_time/end_time to starts_at/ends_at", async () => {
    await POST(mockReq, mockRes)
  ...
  it("returns 400 on missing title", async () => {
    mockReq.body = { starting_price: 100, start_time: "x", end_time: "y", seller_id: "s1" }
    await POST(mockReq, mockRes)
    expect(mockRes.status).toHaveBeenCalledWith(400)
  })

  it("returns 400 on missing seller_id", async () => {
    mockReq.body = { title: "Item", starting_price: 100, start_time: "x", end_time: "y" }
    await POST(mockReq, mockRes)
    expect(mockRes.status).toHaveBeenCalledWith(400)
  })
...
describe("GET /admin/auctions/:id", () => {
  ...
  it("returns auction by id", async () => {
    mockModule.listAuctionListings.mockResolvedValue([{ id: "auc_1", title: "Item" }])
    await GET_ID(mockReq, mockRes)
    expect(mockRes.json).toHaveBeenCalledWith({ item: { id: "auc_1", title: "Item" } })
  })
  ...
  it("queries with correct id filter", async () => {
    ...
    await GET_ID(mockReq, mockRes)
    expect(mockModule.listAuctionListings).toHaveBeenCalledWith({ id: "auc_1" }, { take: 1 })
  })
})
...
describe("POST /admin/auctions/:id (update)", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    mockModule = { updateAuctionListings: jest.fn().mockResolvedValue({ id: "auc_1", title: "Updated" }) }
    mockReq = {
      params: { id: "auc_1" },
      body: { title: "Updated" },
      scope: { resolve: jest.fn().mockReturnValue(mockModule) },
  ...
  it("updates auction successfully", async () => {
    ...
    expect(mockRes.json).toHaveBeenCalledWith({ item: { id: "auc_1", title: "Updated" } })
  })
...
describe("DELETE /admin/auctions/:id", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    mockModule = { deleteAuctionListings: jest.fn().mockResolvedValue(undefined) }
    mockReq = {
      params: { id: "auc_1" },
      scope: { resolve: jest.fn().mockReturnValue(mockModule) },
    }
  ...
  it("passes id as array to delete method", async () => {
    ...
    await DELETE_ID(mockReq, mockRes)
    expect(mockModule.deleteAuctionListings).toHaveBeenCalledWith(["auc_special"])
  })
