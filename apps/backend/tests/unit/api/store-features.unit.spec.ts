...
describe("GET /store/features", () => {
  ...
  it("transforms admin features to public flags when fetch succeeds", async () => {
    const adminFeatures = {
      modules: {
        marketplace: { enabled: true, config: { allowVendorRegistration: true, showVendorPages: true } },
        b2b: { enabled: false, config: { allowCompanyRegistration: false, enableQuotes: false, enableInvoices: false } },
        subscriptions: { enabled: true, config: { showOnProductPages: true, trialEnabled: true, trialDays: 14 } },
        bookings: { enabled: false, config: { showOnHomepage: false } },
        reviews: { enabled: true, config: { showOnProductPages: true, allowPhotos: true } },
        volumePricing: { enabled: false, config: { showOnProductPages: false, showSavingsPercentage: false } },
        wishlists: { enabled: true, config: {} },
        giftCards: { enabled: false, config: {} },
      },
      homepage: { sections: [{ id: "hero", type: "hero", enabled: true, config: {} }] },
      navigation: {
        header: { showCategories: true, showVendors: true, showServices: false, showB2BPortal: false, customLinks: [] },
        footer: { showCategories: true, showVendors: true, showServices: false, customSections: [] },
      },
    }
