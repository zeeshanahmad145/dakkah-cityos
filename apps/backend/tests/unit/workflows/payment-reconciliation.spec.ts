...
describe("Payment Reconciliation Workflow", () => {
  ...
  describe("fetchPaymentRecordsStep", () => {
    it("should fetch payments within the date range", async () => {
      const mockPayments = [{ id: "pay_01" }, { id: "pay_02" }, { id: "pay_03" }]
    ...
    it("should propagate payment module errors", async () => {
      const container = mockContainer({
        payment: { listPayments: jest.fn().mockRejectedValue(new Error("Payment service down")) },
      })
      await expect(fetchPaymentRecordsStep(validInput, { container })).rejects.toThrow("Payment service down")
    })
  })

  describe("matchTransactionsStep", () => {
    it("should match all payments with confidence 1.0", async () => {
      const payments = [{ id: "pay_01" }, { id: "pay_02" }]
      const result = await matchTransactionsStep({ payments })
      expect(result.matched).toHaveLength(2)
      expect(result.matched[0]).toEqual({ payment_id: "pay_01", matched: true, match_confidence: 1.0 })
    })

    it("should report zero unmatched when all match", async () => {
      const payments = [{ id: "pay_01" }]
      const result = await matchTransactionsStep({ payments })
  ...
  describe("reconcileStep", () => {
    it("should create a completed reconciliation record", async () => {
      ...
      const result = await reconcileStep({ matched, tenantId: "tenant_01" })
  ...
  describe("generateReportStep", () => {
    it("should generate a report with period and totals", async () => {
      ...
      const result = await generateReportStep({ reconciliation, dateFrom: "2026-01-01", dateTo: "2026-01-31" })
      expect(result.report.period).toEqual({ from: "2026-01-01", to: "2026-01-31" })
