...
describe("Store Quotes Endpoints", () => {
  describe("POST /store/quotes", () => {
    const validBody = {
      items: [
        { product_id: "prod_01", title: "Widget", quantity: 5, unit_price: 1000 },
      ],
    ...
    it("should create a quote for authenticated customer", async () => {
      ...
      const req = createMockReq({ body: validBody, quoteModuleService }) as any
      const res = createMockRes()

      await POST(req, res)
      expect(mockJson).toHaveBeenCalledWith(expect.objectContaining({ quote: expect.any(Object) }))
    })
    ...
    it("should generate a quote number", async () => {
      ...
      const quoteModuleService = {
        ...
        retrieveQuote: jest.fn().mockResolvedValue({ id: "quote_02", items: [] }),
      }
      const req = createMockReq({ body: validBody, quoteModuleService }) as any
      const res = createMockRes()
    ...
    it("should create quote items for each item in the request", async () => {
      ...
      const quoteModuleService = {
        ...
        createQuoteItems,
        calculateQuoteTotals: jest.fn(),
        retrieveQuote: jest.fn().mockResolvedValue({ id: "quote_03", items: [] }),
      }
      const body = {
        items: [
      ...
      const req = createMockReq({ body, quoteModuleService }) as any
      const res = createMockRes()
  ...
  describe("GET /store/quotes", () => {
    it("should list quotes for authenticated customer", async () => {
      ...
      const req = createMockReq({ quoteModuleService }) as any
      const res = createMockRes()
